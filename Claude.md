...
| `check-completeness` | Every ticket carries an `### Interface` section defining its request and response shapes, and a signature for every use case it names, so two implementers build the same thing |
