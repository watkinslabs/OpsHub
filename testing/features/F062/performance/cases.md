...
- `primitive_bundle_under_90kb_gzipped` — NFR-F062-01: the built `apps/web/src/ui` entry excluding icons and fonts stays under 90 KB gzipped; the check names the largest contributors on failure.
- `tokens_css_under_12kb` — NFR-F062-01: the emitted token and theme CSS stays under 12 KB.
- `ten_thousand_row_scroll_holds_sixty_fps` — NFR-F062-01: scrolling a virtualized 10,000-row `DataTable` holds 60 fps with no frame over 16 ms at the 95th percentile.
...
- `density_switch_causes_no_layout_thrash` — FR-F062-06: switching density triggers one style recalculation, not one per row.
