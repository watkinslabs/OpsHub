...
| `F062-REQ-016` | FR-F062-16 | frontend, e2e | without the `data-grid-pro` entitlement every Community capability works and each Pro affordance is disabled with a tooltip linking to `/admin/entitlements`; the default bundle contains no Pro import and no console error; granting the entitlement enables reorder, pinning, grouping, tree data, aggregation and Excel export |
