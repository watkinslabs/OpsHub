...
| `F062-REQ-016` | FR-F062-16 | frontend, api | every grid capability works at one tier — grouping, tree rows, aggregation and xlsx export call the F013, F009, F022 and F010 endpoints, and resize, reorder, hide, freeze, range selection and clipboard run on the Community grid; the dependency tree and build graph contain no `@mui/x-data-grid-pro` or `-premium`, and no upgrade prompt appears in the grid |
| `F062-REQ-017` | FR-F062-17 | frontend, e2e | reorder by header drag and its keyboard equivalent write F008 `layout.column_order` within 1 s and do not change another user's order; freeze, hide, Shift+Arrow, Shift+Click and Ctrl+Click selection all work; clipboard copy emits TSV that pastes into a spreadsheet with visible formatting; each carries an accessible announcement |
