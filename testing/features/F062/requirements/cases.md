...
| `F062-REQ-016` | FR-F062-16 | frontend, api | every tenant gets every grid capability with no entitlement consulted and no upgrade prompt; the licence key is injected at build time from `mui/x-license-key` and never fetched at runtime; a production build without it fails naming the secret, while development and CI run watermarked on the evaluation without failing |
| `F062-REQ-017` | FR-F062-17 | frontend, e2e | pinning, reorder, range and non-contiguous selection and clipboard TSV all work and persist through F008 `layout` within 1 s without changing another user's layout; grouping, tree rows, aggregation and export come from the F013, F009, F022 and F010 endpoints; each behaviour has a keyboard path and an announcement |
