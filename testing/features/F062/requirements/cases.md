...
| `F062-REQ-016` | FR-F062-16 | frontend, e2e | unentitled, sorting, filtering, resize, selection, pagination, grouping, tree rows and xlsx export all work, and only column reorder, column pinning, row pinning and range selection render disabled with a tooltip linking to `/admin/entitlements`; the default bundle contains no paid import and no console error |
| `F062-REQ-017` | FR-F062-17 | frontend, e2e, performance | entitled, reorder writes the new order to the view's `settings.columns` and another member sees it; pinning persists per viewer; range selection copies TSV that pastes into a spreadsheet; each has a keyboard equivalent and an announcement; the story matrix runs in both entitlement states and the entitled lane reports `skipped: no licence key` rather than passing when the CI secret is absent |
