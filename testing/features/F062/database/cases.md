...
- `theme_and_density_are_browser_state_only` — FR-F062-05, FR-F062-07: the only persistence is `localStorage` keys `opshub.theme` and `opshub.density`; no cookie is set and no request carries the preference.
- `preferences_survive_reload_and_reset_cleanly` — FR-F062-05: a cleared `localStorage` falls back to `system` theme and `comfortable` density without an error or a flash.
