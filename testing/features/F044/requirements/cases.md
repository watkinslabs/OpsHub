...
| `F044-REQ-002` | FR-F044-02 | api | Ticket F012 section 4 drops the backticked route `/api/v1/dependencies` → `contract.route_missing_in_ticket`; drops event `dependency.created.v1` → `contract.event_missing_in_ticket`; both name the ticket path and line; a ticket naming `/public/views/{token}` with no catalog row → `contract.route_undeclared`, while a base path such as `/scim/v2` under declared SCIM routes is accepted |
