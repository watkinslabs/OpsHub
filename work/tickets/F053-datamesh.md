...
conflicts_with: []
parallel_safe: true
owned_paths: [crates/domain/src/datamesh/**, services/api/src/datamesh/**, services/worker/src/datamesh/**, apps/web/src/features/datamesh/**, services/api/migrations/*_datamesh_*.sql, testing/features/F053/**]
...
- Decision references: `docs/architecture-decisions.md` sections 2, 3, 4, 7, 9, 10; `docs/capability-contracts.md` row F053
...
- **FR-F053-01:** An actor with the `data-admin` role can create a mapping with `name` (1–120 chars), `source_sheet_id`, `target_sheet_id` (different sheets in the same tenant), `match_keys` (1–3 `{ source_column_id, target_column_id, normalize: exact | trim | case_insensitive | whitespace | date }`), `field_maps` (1–100), `sync_mode` (`manual` | `on_change` | `scheduled` with cron ≥ 15 minutes), `unmatched_policy` (`create` | `ignore` | `flag`), and `deletion_policy` (`ignore` | `clear` | `flag`); the response returns a UUIDv7 `id` and `version` 1.
- **FR-F053-02:** Each field map is `{ source_column_id, target_column_id, direction: source_to_target | bidirectional, transform: none | expression, expression?, overwrite: always | if_empty | never }`; a column not on its sheet, a type-incompatible pair without a transform, an `expression` that fails the F035 parser or exceeds 200 AST nodes, or a `bidirectional` map with a transform returns `400 invalid` with `field_errors.field_maps[i].<field>`.
- **FR-F053-03:** The match engine computes `datamesh_matches` rows `{ mapping_id, source_row_id, target_row_id, key_hash, match_score }` by normalized key equality; a source row matching more than one target row, or a target row matched by more than one source row, is not written and produces a conflict of kind `ambiguous_match`.
...
- **NFR-F053-02 Security/privacy:** the run acts only with the mapping owner's permissions and re-checks them at execution; preview and conflict payloads exclude columns the caller cannot read; tenant isolation is enforced by a `tenant_id` predicate on every query and by cross-tenant negatives in the harness.
...
- Domain entities in `crates/domain/src/datamesh/`: `Mapping { id, tenant_id, workspace_id, name, source_sheet_id, target_sheet_id, match_keys: Vec<MatchKey>, field_maps: Vec<FieldMap>, sync_mode: SyncMode, unmatched_policy, deletion_policy, owner_id, enabled, last_cursor: Option<Cursor>, version, audit fields, deleted_at }`, `MatchKey { source_column_id, target_column_id, normalize: Normalize }`, `FieldMap { source_column_id, target_column_id, direction, transform: Transform, overwrite: Overwrite }`, `Match { mapping_id, source_row_id, target_row_id, key_hash, match_score }`, `MeshRun { id, tenant_id, mapping_id, mapping_version, trigger: manual | on_change | scheduled, status, source_version_cursor, counts: RunCounts { matched, created, updated, cleared, written_back, conflicts }, error_code, started_at, completed_at, duration_ms, correlation_id }`, `Conflict { id, tenant_id, mapping_id, run_id, kind: ambiguous_match | both_changed | unmatched_source | source_deleted, source_row_id, target_row_id, column_id, source_value, target_value, source_version, target_version, status: open | resolved, resolution, resolved_by, resolved_at }`.
...
- Migration `*_datamesh_*.sql` creates `datamesh_mappings(id uuid pk, tenant_id uuid not null, workspace_id uuid not null, name text not null, source_sheet_id uuid not null, target_sheet_id uuid not null, match_keys jsonb not null, field_maps jsonb not null, sync_mode jsonb not null, unmatched_policy text not null check (unmatched_policy in ('create','ignore','flag')), deletion_policy text not null check (deletion_policy in ('ignore','clear','flag')), owner_id uuid not null, enabled bool not null default true, last_cursor jsonb, version bigint not null default 1, created_by, created_at, updated_by, updated_at, deleted_at, check (source_sheet_id <> target_sheet_id))`, `datamesh_matches(tenant_id, mapping_id uuid references datamesh_mappings(id), source_row_id uuid, target_row_id uuid, key_hash bytea, match_score smallint, computed_at timestamptz, primary key (mapping_id, source_row_id))`, `datamesh_runs(id uuid pk, tenant_id, mapping_id, mapping_version bigint, trigger text, status text check (status in ('queued','running','succeeded','failed')), source_version_cursor jsonb, matched int, created int, updated int, cleared int, written_back int, conflicts int, error_code text, started_at, completed_at, duration_ms int, correlation_id uuid, created_at)`, `datamesh_conflicts(id uuid pk, tenant_id, mapping_id, run_id uuid, kind text check (kind in ('ambiguous_match','both_changed','unmatched_source','source_deleted')), source_row_id uuid, target_row_id uuid, column_id uuid, source_value jsonb, target_value jsonb, source_version bigint, target_version bigint, status text check (status in ('open','resolved')), resolution text, reason text, resolved_by uuid, resolved_at timestamptz, created_at)`.
- Invariants: unique `datamesh_mappings(tenant_id, workspace_id, lower(name)) where deleted_at is null`; unique `datamesh_matches(mapping_id, target_row_id)` so one target row has at most one match; partial unique `datamesh_runs(mapping_id) where status in ('queued','running')`; unique `datamesh_runs(mapping_id, source_version_cursor) where status = 'succeeded'` backs idempotency; partial unique `datamesh_conflicts(mapping_id, source_row_id, target_row_id, column_id, kind) where status = 'open'` prevents duplicate open conflicts.
- Indexes: `datamesh_matches(mapping_id, key_hash)`, `datamesh_runs(mapping_id, created_at desc)`, `datamesh_conflicts(mapping_id, status, created_at desc)`, `datamesh_mappings(tenant_id, source_sheet_id) where enabled and deleted_at is null` for the change listener.
...
- Retention/deletion: mappings soft delete and cascade to matches; runs and conflicts follow F027 tenant retention; rollback drops the four tables.
...
- [ ] Database migration/constraint tests: same-sheet check, target uniqueness in matches, single active run, cursor idempotency index, open-conflict uniqueness, rollback
...
- [ ] Migration file name and owned paths claimed, including `services/worker/src/datamesh/**`
...
- Migration adds `datamesh_mappings`, `datamesh_matches`, `datamesh_runs`, and `datamesh_conflicts`; rollback drops them. Feature is off by default behind `F053_FEATURE` and requires a `datamesh` entitlement.
