...
conflicts_with: []
parallel_safe: true
owned_paths: [crates/domain/src/calendar-app/**, services/api/src/calendar-app/**, apps/web/src/features/calendar-app/**, services/api/migrations/*_calendar-app_*.sql, testing/features/F055/**]
...
- Decision references: `docs/architecture-decisions.md` sections 2, 3, 4, 6, 9, 10; `docs/capability-contracts.md` row F055
...
- **FR-F055-13:** The web app renders month, week, and agenda layouts with a timezone switcher, per-source color legend with toggles, a hidden-sources notice, drag-to-reschedule with keyboard equivalent, a source editor dialog, and a publish dialog showing the feed URL, expiry, and revoke action.
...
- Aggregation: `list_events` loads rows per source through the F013 view query (`GET /api/v1/views/{id}/rows` service function) or the F006 row list with F003 row ACL, restricted to rows whose start column falls in the window (index on `cells` typed date values from F007), then normalizes with `chrono-tz`; sources failing the read check are counted into `hidden_sources`.
...
- Migration `*_calendar-app_*.sql` creates `calendars(id uuid pk, tenant_id uuid not null, workspace_id uuid not null, name text not null, description text, default_timezone text not null, week_start text not null check (week_start in ('monday','sunday')), owner_id uuid not null, version bigint not null default 1, created_by, created_at, updated_by, updated_at, deleted_at)`, `calendar_sources(id uuid pk, tenant_id, calendar_id uuid not null references calendars(id) on delete restrict, kind text not null check (kind in ('sheet','view')), sheet_id uuid, view_id uuid, start_column_id uuid not null, end_column_id uuid, duration_column_id uuid, title_column_id uuid not null, color_column_id uuid, color text, all_day_rule text not null, timezone_source text not null, position int not null, created_at, updated_at)`, `calendar_publications(id uuid pk, tenant_id, calendar_id uuid not null references calendars(id), token_hash bytea not null, expires_at timestamptz not null, include_details bool not null default true, revoked_at timestamptz, created_by, created_at)`.
- Invariants: unique `calendars(tenant_id, workspace_id, lower(name)) where deleted_at is null`; check `(kind = 'sheet' and sheet_id is not null) or (kind = 'view' and view_id is not null)`; check `expires_at <= created_at + interval '30 days'`; unique `calendar_publications(token_hash)`; at most one active publication per calendar via partial unique index `where revoked_at is null`; trigger enforcing ≤ 20 sources per calendar.
- Indexes: `calendars(tenant_id, workspace_id, updated_at desc)`, `calendar_sources(calendar_id, position)`, `calendar_publications(token_hash) where revoked_at is null`, `calendar_publications(expires_at)`; event windows rely on the F007 typed date index on `cells`.
...
- Retention/deletion: calendars soft-delete; publications are deleted with the calendar purge (F027); expired tokens are pruned by the nightly job after 90 days; rollback drops the three tables.
...
- [ ] Database migration/constraint tests: source kind check, 30-day check, single active publication, 20-source trigger, rollback
...
- Depends on: F013 (view row queries and calendar view semantics), F011 (date types, working calendars, reschedule route), F048 (`RequireModule`, entitlement limits); decisions sections 2–4, 6, 9, 10; contracts row F055
- Blocks: none
...
- Migration adds `calendars`, `calendar_sources`, and `calendar_publications`; rollback drops them. Feature is off by default behind `F055_FEATURE` and requires the `calendar-app` entitlement.
