...
**`opshub_action` config**: `{ action_type: "update_fields" | "create_row" | "assign" |
"send_in_app", target_sheet_id?: uuid, target_field?: string, params: object }`. The four members are
F018 `ActionKind` names spelled exactly as F018 spells them (FR-F018-04) — this feature runs F018's
actions, so `update_fields` is plural here as it is there, and the notification action is
`send_in_app`, one of F018's three, rather than an unqualified `notify` matching none of them.
`params` is F018's `ActionSpec` payload for the matching kind, validated by F018's own validator
rather than a second one here; `update_fields` requires both `target_sheet_id` and `target_field`.
A Bridge flow reaches four of F018's twelve kinds: the other eight are reachable from a workflow and
adding one here is an amendment to both tickets, never a widening of this check constraint alone.
...
|---|---|---|---|
| 2026-09-04 | F018 FR-F018-04 | `bridge_flow_step_opshub_actions.action_type` renamed to F018's own names: `update_field` → `update_fields`, `notify` → `send_in_app` | The DDL invented two action names for actions F018 owns and F018's validator executes, so a step would have been validated against a kind it could not name |
