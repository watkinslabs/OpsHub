...
conflicts_with: []
parallel_safe: true
owned_paths: [crates/domain/src/mcp/**, crates/contracts/src/mcp/**, services/mcp/src/mcp/**, services/api/src/mcp/**, apps/web/src/features/mcp/**, services/api/migrations/*_mcp_*.sql, testing/harness/mcp/**, testing/features/F047/**]
...
- Decision references: `docs/architecture-decisions.md` sections 1, 3, 8, 9; `docs/capability-contracts.md` row F047
...
- **FR-F047-08:** `tools/call` for a mutating tool never writes on the first call. It validates arguments, evaluates authorization, computes the change summary, inserts an `mcp_confirmations` row with `status: pending`, `tool`, `arguments_hash` (SHA-256 of the canonical JSON arguments), `summary`, `expires_at = now + 15 minutes`, publishes `mcp.mutation-proposed.v1`, and returns `{ content: [...summary...], isError: true, structuredContent: { code: "confirmation_required", confirmation_id, expires_at, summary } }`. The summary lists `resource_uri`, `operation`, and a `changes` array of `{ field, before, after }` for updates or the full proposed record for creates.
...
- Transport in `services/mcp/src/mcp/`: `main.rs` builds the Axum router with `POST /mcp/v1` and `GET /mcp/v1/sse`; `jsonrpc.rs` handles envelope parsing, batching, and the code map `invalid → -32602`, `denied → -32001`, `not_found → -32002`, `conflict → -32003`, `rate_limited → -32004`, `unavailable → -32005`, malformed JSON `→ -32700`, unknown method `→ -32601`; `dispatch.rs` routes methods to use cases; `sse.rs` bridges the F004 event stream with per-event authorization; `state.rs` holds the manifest, pool, and clock.
...
- Migration `*_mcp_*.sql` creates `mcp_confirmations(id uuid pk, tenant_id uuid not null references tenants(id), actor_id uuid not null references users(id), token_id uuid not null references api_tokens(id), tool text not null, resource_uri text, arguments_hash bytea not null, summary jsonb not null, status text not null default 'pending' check (status in ('pending','approved','consumed','expired','rejected')), approved_by uuid references users(id), approved_at timestamptz, expires_at timestamptz not null, consumed_at timestamptz, version bigint not null default 1, created_at timestamptz not null, correlation_id uuid not null)`, `mcp_audit(id uuid, tenant_id uuid not null, actor_id uuid not null, token_prefix text not null, method text not null, tool text, resource_uri text, arguments_digest bytea, decision text not null check (decision in ('allowed','denied','confirmation_required')), outcome text not null check (outcome in ('ok','error')), error_code text, duration_ms int not null, redacted_field_count int not null default 0, confirmation_id uuid, correlation_id uuid not null, occurred_at timestamptz not null, primary key (id, occurred_at))`, and `mcp_rate_limits(tenant_id uuid not null, token_id uuid not null, bucket text not null check (bucket in ('calls','mutations','search')), tokens numeric not null, capacity int not null, window_seconds int not null, refilled_at timestamptz not null, primary key (tenant_id, token_id, bucket))`.
- Invariants: `mcp_audit` is partitioned monthly by `occurred_at` with three months created ahead, and carries the F003 `audit_immutable` trigger rejecting `UPDATE` and `DELETE`; `consumed_at is not null` exactly when `status = 'consumed'` (check); `approved_at is not null` when `status in ('approved','consumed')` (check); a partial unique index `mcp_confirmations_open_idx on (tenant_id, token_id, tool, arguments_hash) where status = 'pending'` collapses repeated proposals of the identical change into one row.
- Indexes: `mcp_confirmations(tenant_id, status, expires_at)` for the sweeper and the approvals list, `mcp_confirmations(actor_id, status)`, `mcp_audit(tenant_id, occurred_at desc)`, `mcp_audit(tenant_id, actor_id, occurred_at desc)`, `mcp_audit(correlation_id)`, `mcp_rate_limits(refilled_at)` for the bucket sweeper.
...
- Retention/deletion: `mcp_audit` partitions follow the F027 retention policy with a 400-day default and are detached rather than deleted; `mcp_confirmations` rows are purged 30 days after `expires_at` or `consumed_at`; rollback drops the three tables, their partitions, and the trigger.
...
- [ ] Database migration/constraint tests: append-only trigger, status checks, partial unique index on open confirmations, partition creation, rollback
...
- Migration adds `mcp_confirmations`, `mcp_audit` (monthly partitions, append-only), and `mcp_rate_limits`; rollback drops them. Feature is off by default behind `F047_FEATURE`.
