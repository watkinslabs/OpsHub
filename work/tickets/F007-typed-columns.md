...
- **FR-F007-05:** `PATCH /api/v1/columns/{id}` accepts `label`, `description`, `required`, `width`, `hidden`, `settings`, `validation`, and `type`; a `type` change is accepted only when the conversion matrix in section 4 allows it, otherwise the response is `invalid` with `field_errors.type = "unsupported_conversion"`.
- **FR-F007-06:** A `type` change or a `validation` change re-normalizes every cell in the column inside the same transaction for sheets up to 10,000 rows and as an async job above that; the response includes `preview.invalid_count` (cells that will hold state `invalid`) and `preview.mode = "sync" | "async"`.
- **FR-F007-07:** `select` columns own an ordered list of `column_options` with `label`, `color` (one of 12 token names), and `archived`; archiving an option keeps existing cells valid but rejects new writes of that option; `settings.multi = true` allows an array of option IDs in one cell.
- **FR-F007-08:** `number`, `currency`, and `duration` columns store `raw` as the user-entered string, `normalized` as a decimal (scale from `settings.precision`, 0–8) or ISO 8601 duration, and `display` formatted with `settings.currency_code` (ISO 4217) or `settings.format`; a non-numeric input yields validation state `invalid` with code `type_mismatch`.
- **FR-F007-09:** `date` and `datetime` columns normalize to ISO 8601 (`YYYY-MM-DD` or RFC 3339 in UTC) and accept `settings.format` for display; `person` columns store a user ID that must belong to the tenant, otherwise state `invalid` with code `unknown_person`.
- **FR-F007-10:** Validation rules are `required`, `min`/`max` (number, currency, duration, date, datetime), `regex` (text, RE2 syntax, ≤ 512 chars), `allowed_options` (select), `date_range`, and `unique` (text, number); each rule failure records state `invalid` with the rule name as `code` and a message in `cell_validation_states`.
...
- **FR-F007-15:** `formula` and `link` columns are created here as shells with `settings.expression = null` or `settings.link_target = null`; their expressions and targets are set only through F035 and F009 routes, and cells of these types are read-only through cell writes.
...
- Indexes: `columns(sheet_id, position) where deleted_at is null`, `columns(tenant_id, id)`, `column_options(column_id, position)`, `cell_validation_states(column_id, state)` for invalid counts, `cell_validation_states(tenant_id, row_id)`.
- The F006 `cells` table gains `normalized jsonb` through an additive nullable column in this migration, with the F006 down migration untouched.
...
- Retention/deletion: soft delete sets `deleted_at`; purge follows F027; rollback drops `column_options`, `cell_validation_states`, `columns`, and the `cells.normalized` column.
...
- Migration adds `columns`, `column_options`, `cell_validation_states`, and `cells.normalized`; rollback removes them. Feature is off by default behind `F007_FEATURE`.
