...
conflicts_with: []
parallel_safe: true
owned_paths: [crates/domain/src/resources/**, services/api/src/resources/**, apps/web/src/features/resources/**, services/api/migrations/*_resources_*.sql, testing/features/F033/**]
...
- Decision references: `docs/architecture-decisions.md` sections 2, 3, 4, 9; `docs/capability-contracts.md` row F033
...
- **FR-F033-02:** `GET /api/v1/resources` pages by opaque cursor with `limit` 1–200, filters `status`, `kind`, `skill` (name with optional `min_level`), `role_title` prefix, and `available_between` (`from`, `to`, `min_hours`), and sorts by `display_name` or `updated_at`; `PATCH /api/v1/resources/{id}` updates profile fields and `cost_rates` with `If-Match`.
- **FR-F033-03:** `PUT /api/v1/resources/{id}/skills` replaces the skill set with up to 50 entries of `{ skill (1–80 chars), level 1–5 }`; duplicate skill names return `invalid` with `field_errors.skills[i]`.
...
- **FR-F033-05:** Cost rates are an ordered list of `{ hourly_rate (0–100,000, two decimals), currency (ISO 4217), effective_from, effective_to? }` per resource with no overlapping effective ranges; the rate effective on an allocation's `start_date` is snapshotted into the allocation as `cost_rate_snapshot` and the allocation exposes `planned_cost = planned_hours × hourly_rate`.
...
- Domain entities in `crates/domain/src/resources/`: `Resource { id, tenant_id, user_id, display_name, kind: ResourceKind, role_title, working_calendar_id, fte: Decimal, timezone, status: ResourceStatus, version, audit fields, deleted_at }`, `ResourceSkill { resource_id, skill, level: u8 }`, `AvailabilityEntry { id, resource_id, kind: AvailabilityKind, start_date, end_date, hours_per_day: Option<Decimal>, note }`, `CostRate { id, resource_id, hourly_rate: Decimal, currency, effective_from, effective_to }`, `Allocation { id, tenant_id, resource_id, project_sheet_id, row_id, start_date, end_date, planned: Planned (Hours | Percent), role, confidence: Confidence, cost_rate_snapshot: Option<RateSnapshot>, note, version, audit fields, deleted_at }`, `CapacityPeriod { start, end, calendar_hours, fte_hours, leave_hours, holiday_hours, reduced_hours, available_hours, allocated_hours, remaining_hours, over_allocated, warnings }`.
...
- Migration `*_resources_*.sql` creates `resources(id uuid pk, tenant_id, user_id uuid, display_name text not null, kind text not null, role_title text, working_calendar_id uuid not null, fte numeric(3,2) not null default 1.00, timezone text not null, status text not null default 'active', version bigint, audit fields, deleted_at)`, `resource_skills(tenant_id, resource_id, skill text, level smallint not null, primary key (resource_id, skill))`, `resource_availability(id uuid pk, tenant_id, resource_id, kind text not null, start_date date not null, end_date date not null, hours_per_day numeric(4,2), note text)`, `cost_rates(id uuid pk, tenant_id, resource_id, hourly_rate numeric(12,2) not null, currency char(3) not null, effective_from date not null, effective_to date)`, `allocations(id uuid pk, tenant_id, resource_id, project_sheet_id, row_id, start_date date not null, end_date date not null, planned_hours numeric(10,2), planned_percent smallint, role text, confidence text not null, cost_rate_snapshot jsonb, note text, version bigint, audit fields, deleted_at)`.
- Invariants: unique partial index `resources_tenant_user_idx on (tenant_id, user_id) where status = 'active' and deleted_at is null and user_id is not null`; check `fte between 0.05 and 1.00`; check `kind in ('person','placeholder')`; check `level between 1 and 5`; exclusion constraint `resource_availability_no_overlap exclude using gist (resource_id with =, daterange(start_date, end_date, '[]') with &&)`; exclusion `cost_rates_no_overlap` on `(resource_id, daterange(effective_from, coalesce(effective_to, 'infinity'), '[]'))`; check `(planned_hours is null) <> (planned_percent is null)`; check `end_date >= start_date and end_date - start_date <= 366`; check `confidence in ('committed','likely','tentative')`; foreign keys to `resources(id)`, `sheets(id)`, `rows(id)`, `working_calendars(id)` with `on delete restrict`.
- Indexes: `resources(tenant_id, status, display_name)`, `resource_skills(tenant_id, skill, level)`, `allocations(resource_id, start_date, end_date) where deleted_at is null`, `allocations(project_sheet_id) where deleted_at is null`, `allocations(row_id)`, gist `allocations using gist (resource_id, daterange(start_date, end_date, '[]'))`.
...
- Retention/deletion: resources and allocations soft-delete; purge via F027; migration rollback drops the five tables and the `btree_gist` extension dependency remains.
...
- [ ] Database migration/constraint tests: user uniqueness, exclusion constraints, planned check, range check, foreign keys, rollback
...
- Migration adds `resources`, `resource_skills`, `resource_availability`, `cost_rates`, and `allocations`; rollback drops them. Feature is off by default behind `F033_FEATURE`.
