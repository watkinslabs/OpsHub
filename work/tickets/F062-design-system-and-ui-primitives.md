...
- **FR-F062-10:** MUI X supplies the three data surfaces rather than hand-built equivalents: `DataGrid` backs the sheet grid, admin tables, and every list (virtualization above 100 rows, sticky header, column resize and reorder, selection, server-side pagination bound to the F028 cursor), `Charts` backs every widget in F022–F024, and `DatePickers` backs every date and range field with the F049 locale and timezone. Each is wrapped once in `apps/web/src/ui/data/` to bind theme, density, locale, and empty and error states; features use the wrapper, never the raw import.
...
- Dependencies: `@mui/material` v7 with `@emotion/react` and `@emotion/styled`, `@mui/x-data-grid`, `@mui/x-charts`, `@mui/x-date-pickers`. MUI's CSS-variables theme mode is enabled so theme and density switch without re-rendering the tree.
...
- Risks and mitigations: MUI major-version churn, mitigated by consuming it only through the re-export surface and the three wrappers so an upgrade touches one directory; MUI X licensing for the Pro-tier Data Grid features, to be confirmed against the columns F008 needs before implementation starts; a token change silently restyling fifty-nine features, mitigated by pinned visual baselines failing on a 0.1% diff; features drifting by importing MUI directly or writing local CSS, mitigated by the lint rules in section 4 and the owned-path gate; over-theming MUI until upgrades are painful, mitigated by keeping overrides to the token mapping and default sizes rather than restyling internals
- Open questions: whether the MUI X Data Grid tier F008 requires is Community or Pro — resolve before F008 planning, since it is a licence purchase, not a code change
...
- [ ] MUI v7, MUI X, Plus Jakarta Sans and JetBrains Mono vendored, and the Data Grid tier question above answered

## 9. Exit criteria — accepted and releasable

- [ ] All FR/NFR acceptance tests pass in targeted and full modes
- [ ] React component, E2E, accessibility, visual, and performance gates pass in both themes and both densities
- [ ] Token parity and computed contrast pass for every brand preset; no raw color, spacing, radius, or duration literal exists under `apps/web/src/**`; no feature imports MUI directly
