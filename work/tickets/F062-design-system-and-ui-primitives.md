...
- **FR-F062-16:** MUI X Pro capabilities are an upgrade, never a hard dependency. `DataGridPanel` reads the `data-grid-pro` entitlement through the F048 evaluate endpoint and enables column reorder, column pinning, row grouping, tree data, aggregation, and Excel export only when it is `active` or an unexpired `trial`; the Pro bundle and its licence key load lazily on that path alone, so a Community deployment ships and runs without either. When the entitlement is absent the grid renders every Community capability normally and each Pro affordance appears as a disabled control with a tooltip naming the capability and linking to `/admin/entitlements`; nothing is hidden, no feature silently vanishes, and no console error is produced. The licence key is a tenant setting entered on that admin page, stored encrypted, never sent to the browser of a tenant without the entitlement, and validated on save with the failure reason surfaced. Until F048 ships, the seam resolves to "not entitled" through the documented fallback in section 4, so F062 is releasable in M1 on Community alone.
...
Scenario: Pro capabilities degrade honestly without the entitlement
  Given a tenant without the data-grid-pro entitlement
  When a sheet grid renders
  Then sorting, filtering, resize, selection and pagination all work
  And column reorder and pinning appear disabled with a tooltip linking to /admin/entitlements
  And the loaded bundle contains no Pro import and logs no error
