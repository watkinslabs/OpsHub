...
- **FR-F062-16:** The paid grid entitlement covers presentation only. Row grouping (F013 `settings.group_by`), tree data (F009 `row_hierarchy`), aggregation (F022 metrics) and Excel export (F010 `POST /api/v1/exports` with `format: xlsx`) are OpsHub server capabilities that every tenant gets at every tier; `DataGridPanel` renders what those endpoints return and triggers those jobs, and must never re-implement any of them client-side. What the `data-grid-pro` entitlement adds is the four presentation enhancements the Community grid lacks: column reorder, column pinning, row pinning, and range selection with clipboard copy. `DataGridPanel` reads the entitlement through the F048 evaluate endpoint and enables those four only when it is `active` or an unexpired `trial`; the paid bundle and its licence key load lazily on that path alone, so a Community deployment ships and runs without either. Unentitled, every other capability works unchanged and each of the four affordances appears as a disabled control with a tooltip naming it and linking to `/admin/entitlements`; nothing is hidden and no console error is produced. The licence key is a tenant setting entered on that admin page, stored encrypted, never sent to the browser of a tenant without the entitlement, and validated on save with the failure reason surfaced. Until F048 ships, the seam resolves to "not entitled" through the documented fallback in section 4, so F062 is releasable in M1 on Community alone.
- **FR-F062-17:** The entitled path is built and kept working, not stubbed behind the gate. Column reorder persists the column order to the active F013 view's `settings.columns` on drop, so it survives reload and is shared with everyone using that view; column and row pinning persist per viewer in `localStorage` under `opshub.grid.<view_id>`; range selection supports shift-click and shift-arrow extension and copies TSV to the clipboard that pastes into a spreadsheet with the grid's visible formatting; each of the four has a keyboard equivalent and an accessible announcement, and each degrades to its disabled affordance without the entitlement. Every story in the matrix runs in both entitlement states, and CI supplies the licence key from a secret: when the secret is absent the entitled lane reports `skipped: no licence key` and fails the job if it was expected, so the paid path can never rot into a silent pass.
...
Excluded: selling, pricing, and billing for the `data-grid-pro` entitlement and the entitlement store itself (F048 — this feature only reads it and degrades honestly); row grouping (F013), tree data (F009), aggregation (F022) and file export (F010), which are server capabilities this feature renders but never re-implements; feature screens themselves (each feature ticket), workspace navigation content and routing (F005), locale and timezone resolution (F049 — consumed here with a documented fallback), sheet-specific grid behavior such as formulas and cell editors (F008 composes the `DataGrid` wrapper), chart data queries and widget definitions (F022–F024 use the palette and wrapper), sharing and publishing surfaces (F036, F059, F061), and any marketing or brand site styling.
...
- Dependencies: `@mui/material` v7 with `@emotion/react` and `@emotion/styled`, `@mui/x-data-grid` (Community), `@mui/x-charts`, `@mui/x-date-pickers`. `@mui/x-data-grid-pro` is an optional dependency imported only inside `ui/data/proGrid.ts` behind a dynamic `import()`, so it is absent from the default bundle and a deployment that never buys it never downloads it. MUI's CSS-variables theme mode is enabled so theme and density switch without re-rendering the tree.
- `ui/data/useProEntitlement.ts` is the single seam: it calls `GET /api/v1/feature-flags/evaluate?modules=data-grid-pro` (F048) and returns `{ allowed, reason }`. Before F048 exists the module is not registered, the call is not made, and the hook returns `{ allowed: false, reason: "not_entitled" }` — the same shape, so no code changes when F048 lands.
...
Scenario: Paid enhancements degrade honestly without the entitlement
  Given a tenant without the data-grid-pro entitlement
  When a sheet grid renders
  Then sorting, filtering, resize, selection, pagination, grouping, tree rows and xlsx export all work
  And column reorder, column pinning, row pinning and range selection appear disabled with a tooltip linking to /admin/entitlements
  And the loaded bundle contains no paid import and logs no error

Scenario: Column reorder persists to the view for everyone
  Given a tenant with the data-grid-pro entitlement active
  When a viewer drags a column to a new position on a shared view
  Then the view's settings.columns records the new order
  And another member opening that view sees the same order
...
- Risks and mitigations: MUI major-version churn, mitigated by consuming it only through the re-export surface and the three wrappers so an upgrade touches one directory; a feature quietly assuming a paid capability, mitigated by the disabled-affordance rule in FR-F062-16 and a test asserting the Community bundle carries no paid import; the paid path rotting because nobody runs it, mitigated by the dual-state story matrix and the explicit skip-with-reason in FR-F062-17; a token change silently restyling fifty-nine features, mitigated by pinned visual baselines failing on a 0.1% diff; features drifting by importing MUI directly or writing local CSS, mitigated by the lint rules in section 4 and the owned-path gate; over-theming MUI until upgrades are painful, mitigated by keeping overrides to the token mapping and default sizes rather than restyling internals
- Open questions: none. The Data Grid tier is settled: every deployment ships MUI X Community, and the four presentation enhancements in FR-F062-16 are sold per tenant as the `data-grid-pro` entitlement in `/admin/entitlements` (F048 FR-F048-01, FR-F048-14). Grouping, tree data, aggregation and Excel export are not gated — they are F013, F009, F022 and F010 server capabilities available to every tenant. F008 must design the sheet grid so that column reorder, column pinning, row pinning and range selection are enhancements, never requirements.
...
- [ ] MUI v7, MUI X Community, Plus Jakarta Sans and JetBrains Mono vendored; `@mui/x-data-grid-pro` declared optional and excluded from the default bundle

## 9. Exit criteria — accepted and releasable

- [ ] All FR/NFR acceptance tests pass in targeted and full modes
- [ ] React component, E2E, accessibility, visual, and performance gates pass in both themes and both densities
- [ ] Token parity and computed contrast pass for every brand preset; no raw color, spacing, radius, or duration literal exists under `apps/web/src/**`; no feature imports MUI directly; the default bundle contains no Pro import and the app runs with the entitlement absent
