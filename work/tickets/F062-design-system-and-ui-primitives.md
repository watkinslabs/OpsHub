...
- Lint: the ESLint rule set that makes `docs/engineering-standards.md` enforceable rather than advisory. It bans, under `apps/web/src/**`: raw colour, spacing, radius, type-size and duration literals; any stylesheet or CSS module outside `apps/web/src/design/`; direct imports of `@mui/material`, `@mui/x-*`, `@tanstack/*` or an icon package outside `apps/web/src/ui`; redefinition inside `apps/web/src/features/**` of a component name the library re-exports; `sx` keys that set colour, typography, border or shadow inside `apps/web/src/features/**`, since a visual variation belongs to the component as a named prop rather than to its caller; `dangerouslySetInnerHTML`; and `any`, non-null `!` assertions and silencing `as` casts in application code. Every rule reports the standard's section so the fix is findable, and the `web` CI job fails on any of them.
