...
conflicts_with: []
parallel_safe: true
owned_paths: [crates/domain/src/dependencies/**, crates/persistence/src/dependencies/**, services/api/src/dependencies/**, apps/web/src/features/dependencies/**, services/api/migrations/*_dependencies_*.sql, testing/features/F012/**]
...
- Persistence (`crates/persistence/src/dependencies/`): `RowDependencyRepository` owns `row_dependencies`; `ScheduleResultRepository` owns `schedule_results`. Each implements the shared `Repository` contract (`get`, `list` with cursor pagination, `insert`, `update` under an expected version, `soft_delete`, `restore`, `purge`) and adds named queries `list_for_sheet(sheet_id)`, `find_pair(predecessor_row_id, successor_row_id)`, `load_graph_for_sheet(sheet_id)`, `delete_for_sheet`, `upsert_schedule_results(sheet_id, rows)`, `list_critical(sheet_id)`; the tenant predicate, soft-delete filter, version check, audit row, and outbox enqueue come from the base contract. The per-sheet row lock is F011's `SheetScheduleSettingsRepository::lock_for_schedule(sheet_id)`, called by the use case inside the `UnitOfWork`. Multi-table writes — dependency create/update/delete with their audit and outbox rows, and a committed shift's cell writes plus the `schedule_results` rewrite — run in one `UnitOfWork` that owns the transaction. Row start/end dates are read and written through the F006/F007 cell repositories and F011's reschedule use case, never through F012 SQL. Per decision 2.1 the use cases above depend on these repository traits and contain no SQL: no SQL string, `sqlx::query*` call, or connection exists in `crates/domain/src/dependencies` or `services/api/src/dependencies`; `detect_cycle` and `compute_critical_path` operate on the in-memory graph returned by `load_graph_for_sheet` and contain no SQL.
- Recompute trigger: the consumer subscribes to `row.updated.v1`, `cell.updated.v1`, and `row.reparented.v1` for sheets with schedule settings and rewrites the sheet's results through `ScheduleResultRepository::upsert_schedule_results`, bumping `schedule_version`.
...
- `schedule_results` is a derived, rebuildable cache of the critical-path computation: it serves `GET /api/v1/sheets/{sheet_id}/critical-path` and the Gantt critical-path overlay, and is rebuilt by the schedule recompute specified above (the `row.updated.v1`, `cell.updated.v1`, `row.reparented.v1` consumer, debounced 500 ms per sheet) and by each committed shift. It is never the source of truth: row start and end dates live in the F006/F007 cell tables, and a dropped or stale `schedule_results` row is recomputed from those cells and `row_dependencies`.
- Invariants: unique index `row_dependencies_pair_idx on (tenant_id, predecessor_row_id, successor_row_id)`; check constraint on lag range; trigger-free cycle safety is guaranteed by the create/update use case taking the sheet's `sheet_schedule_settings` row lock through F011's `SheetScheduleSettingsRepository::lock_for_schedule(sheet_id)` inside the `UnitOfWork`, so concurrent inserts serialize per sheet exactly as the previous inline `SELECT ... FOR UPDATE` did — the same lock on the same row, only the SQL moved into `crates/persistence`.
...
- Risks and mitigations: graph work on every cell edit could be expensive, so recompute is debounced per sheet by 500 ms in the consumer and bounded by the 2 s budget; concurrent dependency inserts could bypass cycle detection, so inserts take the sheet's schedule settings row lock through `lock_for_schedule(sheet_id)`; hour-based lag across calendar exceptions is easy to get wrong, so working-hour arithmetic stays in F011 and F012 only calls it.
- Open questions: none
