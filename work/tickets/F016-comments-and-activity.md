...
- **FR-F016-01:** An actor with `resource-commenter` or higher on the target can `POST /api/v1/comments` with `{ target_kind, target_id, column_id?, body, thread_id?, parent_comment_id? }` where `target_kind` is one of `sheet`, `row`, `cell`, `file`, `report`, `dashboard`, and `column_id` is required when `target_kind` is `cell` (where `target_id` is the row id) and rejected otherwise; the response returns the comment with UUIDv7 `id`, `thread_id`, `version` 1, and `created_at`.
...
- **FR-F016-05:** `GET /api/v1/{target_kind}/{target_id}/comments` — with a required `column_id` query parameter when `target_kind` is `cell`, where `{target_id}` is the row id — returns threads with nested comments in `created_at` order, cursor paged with `limit` 1–100 threads, filterable by `resolved=true|false`, and includes `resolved_at`, `resolved_by`, and per-comment `edited_at`.
...
column_id: uuid? }` — the target addressed by the two path segments of the list routes and carried in
every response. Access is the target's own ACL: a target the actor cannot read is `404 not_found` on
every route, so a comment count never confirms that a record exists.

A cell has no id of its own — it is the pair `(row_id, column_id)` (F007) — so for `kind = "cell"`,
`id` is the **row** id and `column_id` is required; for every other kind `column_id` must be absent.
Either mistake is `400 invalid` with `field_errors.column_id = "required"` or `"not_applicable"`. On
the list routes the segment `{target_id}` is the row id and `column_id` travels as a required query
parameter, so the path stays the same for all six kinds. A cell thread is scoped to its column: it is
not returned by the row's own listing, and deleting the column soft-deletes its cell threads through
the same cascade as deleting the row.
...
| `column_id` | uuid? | yes for `kind = "cell"` | a live, non-deleted column of the row's sheet; required with `cell` and rejected with every other kind (`field_errors.column_id`); an unknown or foreign-tenant column is `404 not_found` |
...
| `201` | comment created |
| `204` | comment deleted |
| `400 invalid` | `too_long`, `too_many_mentions`, `target_mismatch`, `thread_mismatch`, a `column_id` that is `required` for `cell` or `not_applicable` for another kind, an unknown `target_kind`, a `limit` outside its route bounds, `until` before `since`, an unlisted field |
...
|---|---|---|---|
| 2026-09-04 | Interface review | `TargetRef` gained `column_id`; `target_column_id` added to `comment_threads` and `activity_entries` with a check constraint tying it to `target_kind = 'cell'`; the list routes take `column_id` as a required query parameter for cells | `target_kind = "cell"` was in the enum but a cell has no id of its own, so no implementer could have addressed one; the target is the pair `(row_id, column_id)` |
