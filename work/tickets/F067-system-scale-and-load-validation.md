...
- The generator writes through SQLx to the load environment's database using the schema owned by other features (`tenants`, `users`, `groups`, `group_members` from F002; `sheets`, `rows`, `cells` from F006), in dependency order, inside one transaction per tenant batch of 100, with `session_replication_role = replica` during load so foreign keys are validated once at the end rather than per row.
