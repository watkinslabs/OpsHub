...
conflicts_with: []
parallel_safe: true
owned_paths: [crates/domain/src/workflows/**, services/api/src/workflows/**, apps/web/src/features/workflows/**, services/api/migrations/*_workflows_*.sql, testing/features/F018/**]
...
- **FR-F018-07:** `POST /api/v1/workflows/{id}/publish` validates the full definition, writes an immutable `workflow_versions` row with a monotonically increasing `version_no`, sets `state: published`, `published_version` to that row, emits `workflow.published.v1`, and returns the new version; publishing a definition with validation errors returns `invalid` with every error listed.
...
- Invariants: unique `workflow_versions(workflow_id, version_no)`; unique `workflow_steps(version_id, index)`; `workflow_versions` rows have no `updated_at` and a trigger `workflow_versions_immutable` raises on `UPDATE` or `DELETE`; unique `workflows(tenant_id, sheet_id, lower(name)) where deleted_at is null`; `published_version_id` references `workflow_versions(id)`.
- Indexes: `workflows(tenant_id, sheet_id, state) where deleted_at is null`, `workflows(tenant_id, updated_at desc)`, `workflow_versions(workflow_id, version_no desc)`, `workflow_steps(version_id, index)`.
...
- Retention/deletion: soft delete sets `deleted_at` on `workflows`; versions are retained while any `workflow_runs` row (F019) references them and purged by the F027 job otherwise; rollback drops the three tables.
...
- [ ] Database migration/constraint tests: immutable version trigger, unique version number, unique name, rollback
...
- Depends on: F007 (typed columns for operator validation), F035 (formula leaves); decisions sections 2–4, 6; contracts row F018
- Blocks: F019, F040
...
- Migration adds `workflows`, `workflow_versions`, and `workflow_steps`; rollback drops them. Feature is off by default behind `F018_FEATURE`.
