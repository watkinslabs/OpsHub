...
- **FR-F009-06:** `PUT /api/v1/columns/{id}/rollup` stores one `rollup_rules` row for a non-formula column with `function` one of `sum|min|max|avg|count|any|all|first|last|weighted_percent`, `source_column_id` (same sheet, type compatible with the function), optional `filter` (typed comparison on a sibling column), and for `any|all` on `select` columns an ordered `status_priority` list; a `weighted_percent` rule additionally names `weight_column_id` of type `number` or `duration`; `{ function: null }` removes the rule and emits `rollup.recomputed.v1` with `cell_count` 0.
- **FR-F009-07:** After any `row.updated.v1`, `cell.updated.v1`, `cells.bulk-updated.v1`, `rows.bulk-updated.v1`, `row.reparented.v1`, `row.deleted.v1`, or `row.restored.v1` event on a sheet with roll-up rules, the consumer recomputes only the ancestors of the changed rows for the affected columns, writes each parent cell `raw`, `display`, and `validation.state = valid`, and emits one `rollup.recomputed.v1` per column with `cell_count` and `duration_ms`.
...
- Validation: depth ≤ 20; `limit` 1–500; `link_type` and `sync_direction` enums; roll-up function/type matrix (`sum|avg|min|max` on number, currency, duration; `min|max` also on date and datetime; `count` on any; `any|all|first|last` on select and boolean; `weighted_percent` on number with a number or duration weight); `filter` references a column of the same sheet.
...
- Retention/deletion: hierarchy rows follow the row's soft delete; link soft delete sets `deleted_at`; purge from F027 removes both with their rows; rollback drops the three tables and the trigger.
...
- Migration adds `row_hierarchy`, `cell_links`, and `rollup_rules`; rollback drops them. Feature is off by default behind `F009_FEATURE`.
