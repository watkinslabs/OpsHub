...
| `state` | `"ok"\|"missing"\|"denied"\|"error"` | row-level outcome, the same four members every measure `state` takes. A project soft-deleted since the last refresh keeps its row at `missing` with `reason: "project_deleted"` and every measure `missing` too (FR-F031-14), rather than disappearing from a portfolio nobody edited |
...
|---|---|---|---|
| 2026-09-04 | Interface review | `portfolio_rollup_rows.row_state` gained `missing`, matching the four members every measure `*_state` already takes | FR-F031-14 requires a deleted project's row to be marked `missing`, a value the check constraint forbade, so the requirement was unimplementable as written |
