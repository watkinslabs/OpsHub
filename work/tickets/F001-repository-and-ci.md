...
- **FR-F001-07:** The `validate-work` job runs `cargo xtask validate-work`, `validate-plan`, `validate-tickets`, `check-contracts`, `check-migrations`, and `check-persistence` in that order and exits 1 with the `BLOCKED:` lines from the first failing command.
