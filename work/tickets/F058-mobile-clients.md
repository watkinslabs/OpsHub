...
conflicts_with: []
parallel_safe: true
owned_paths: [crates/domain/src/mobile/**, services/api/src/mobile/**, apps/web/src/features/mobile/**, services/api/migrations/*_mobile_*.sql, testing/features/F058/**]
...
- Decision references: `docs/architecture-decisions.md` sections 2, 3, 4, 5, 6; `docs/capability-contracts.md` row F058
- Module slug: `mobile`
...
- **FR-F058-04:** `POST /api/v1/mobile/sync` accepts `{ device_id, batch_id, ops: [{ client_op_id, kind: cell_edit|form_submit, target, base_version, payload, recorded_at }] }` (≤ 500 ops), applies each op in recorded order through the F008 and F014 domain services, and returns `{ applied: [{ client_op_id, version }], rejected: [{ client_op_id, code, server_value, server_version }], cursor }`.
- **FR-F058-05:** A cell edit whose `base_version` is older than the current row version and whose target cell changed since is rejected with code `conflict`; an edit to a row the actor can no longer edit is rejected with `denied`; an edit to a deleted row is rejected with `not_found`; rejections persist in `mobile_sync_rejections` and publish `mobile-sync.rejected.v1`.
- **FR-F058-06:** Replaying a batch with the same `batch_id` or ops with already-applied `client_op_id` values returns the original results without re-applying; applied ops publish `mobile-sync.applied.v1` once per batch with `applied_count` and `rejected_count`.
- **FR-F058-07:** `GET /api/v1/mobile/sync?since={cursor}` returns rows changed since the cursor for the sheets the device has opened in the last 30 days (`mobile_devices.subscriptions`), paged with `limit` ≤ 500, including `deleted` markers and the new cursor; a cursor older than 7 days returns `invalid` with `field_errors.since = "expired"` and the client performs a full refresh.
...
- **FR-F058-10:** Push notifications delivered by F037 to a registered device carry `deep_link`, and tapping the notification opens `/m/{deep_link}` inside the installed app (service worker `notificationclick`) and marks the notification read through F037.
...
- **NFR-F058-04 Reliability/observability:** sync is idempotent per `batch_id` and `client_op_id`; metrics `mobile_sync_ops_total{result}`, `mobile_sync_batch_duration_ms`, and `mobile_queue_depth` (client telemetry) are emitted; every batch carries a `correlation_id` shared by all applied ops.
...
- Canonical contract: aggregate `mobile-session`; module `mobile`; routes `GET /manifest.webmanifest`, `POST /api/v1/mobile/sync`, `GET /api/v1/mobile/sync?since={cursor}`, `POST /api/v1/mobile/devices`, `DELETE /api/v1/mobile/devices/{id}`, `GET /m/{deep_link}`; events `mobile-sync.applied.v1`, `mobile-sync.rejected.v1`, `mobile-device.registered.v1`; tables `mobile_devices`, `mobile_sync_batches`, `mobile_sync_rejections`; roles `resource-editor` (applied ops), `self` (devices).
- Domain entities in `crates/domain/src/mobile/`: `MobileDevice { id, tenant_id, user_id, session_id, platform, push_subscription_id, app_version, device_name, subscriptions: Vec<SheetSubscription>, last_sync_cursor, revoked_at, version, audit fields }`, `SyncBatch { id, tenant_id, device_id, batch_id, applied_count, rejected_count, cursor, correlation_id, created_at }`, `SyncOp { client_op_id, kind: OpKind, target: OpTarget, base_version, payload, recorded_at }`, `SyncRejection { id, batch_id, client_op_id, code, server_value, server_version }`, `DeepLink { kind, id, signature, expires_at }`.
...
- Migration `*_mobile_*.sql` creates `mobile_devices(id uuid pk, tenant_id uuid not null, user_id uuid not null, session_id uuid not null, platform text not null check (platform in ('android','ios','web')), push_subscription_id uuid, app_version text not null, device_name text not null, subscriptions jsonb not null default '[]', last_sync_cursor text, revoked_at timestamptz, version bigint not null default 1, created_by, created_at, updated_by, updated_at)`, `mobile_sync_batches(id uuid pk, tenant_id, device_id references mobile_devices(id) on delete restrict, batch_id uuid not null, applied_count int not null, rejected_count int not null, cursor text not null, correlation_id uuid not null, response jsonb not null, created_at)`, `mobile_sync_rejections(id uuid pk, tenant_id, batch_id uuid not null, device_id uuid not null, client_op_id uuid not null, code text not null, target jsonb not null, server_value jsonb, server_version bigint, created_at)`.
- Invariants: unique `mobile_sync_batches_device_batch_idx on (device_id, batch_id)`; unique `mobile_sync_applied_ops_idx on (device_id, client_op_id)` maintained through a companion table `mobile_sync_applied_ops(device_id, client_op_id, version, created_at)` created in the same migration; check `applied_count + rejected_count <= 500`; one active device per `(user_id, session_id)` partial unique index where `revoked_at is null`.
- Indexes: `mobile_devices(tenant_id, user_id) where revoked_at is null`, `mobile_sync_rejections(device_id, created_at desc)`, `mobile_sync_batches(device_id, created_at desc)`.
...
- Retention/deletion: batches and rejections older than 30 days are purged by the F027 job; revoked devices are purged after 90 days; rollback drops the four tables.
...
- [ ] Database migration/constraint tests: batch uniqueness, applied-op uniqueness, active device index, rollback
...
- Migration adds `mobile_devices`, `mobile_sync_batches`, `mobile_sync_applied_ops`, and `mobile_sync_rejections`; rollback drops them. Feature is off by default behind `F058_FEATURE`.
