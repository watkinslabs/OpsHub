...
- **NFR-F004-03 Accessibility:** operator-facing output is usable without colour or a screen: `/readyz` JSON and CLI output use words (`ok`, `error` at the top level; `ok`, `degraded`, `unreachable` per component) not colour alone, respect `NO_COLOR`, and the runbooks in `infra/backup/` use heading hierarchy and plain-text tables readable by screen readers.
...
|---|---|---|---|
| 2026-09-04 | Interface review | FR-F004-14 and NFR-F004-03 now use the two vocabularies this ticket actually defines: `status` is `ok`/`error`, component `state` is `ok`/`degraded`/`unreachable` | FR-F004-14 said a failing component is marked `"error"`, a value no component `state` may hold, so the requirement and the response shape disagreed |
