...
conflicts_with: []
parallel_safe: true
owned_paths: [crates/domain/src/workload/**, crates/persistence/src/workload/**, services/api/src/workload/**, services/worker/src/workload/**, apps/web/src/features/workload/**, services/api/migrations/*_workload_*.sql, testing/features/F034/**]
...
- **FR-F034-02:** The worker consumes `capacity.computed.v1` from F033 and, for the affected resource and span, upserts one `workload_conflicts` row per period where `allocated_hours > available_hours` with `over_hours` and `status: open`, writes one `workload_conflict_allocations` row per contributing allocation and deletes the rows for allocations that no longer contribute, publishes `workload-conflict.detected.v1` for each newly opened conflict, and sets `status: resolved` with `resolved_at` for periods no longer over-allocated.
- **FR-F034-03:** `GET /api/v1/workload/conflicts?resource_id&project_sheet_id&status&from&to` pages open and resolved conflicts, returns each conflict's contributing allocations as an `allocation_ids` array assembled from its `workload_conflict_allocations` rows, and includes `suggestions` per conflict: contributing allocations whose task row has `total_float_days > 0` in F012 `schedule_results` (`shift_within_float`), and up to three alternative active resources sharing a required skill with `remaining_hours ≥ over_hours` in that period (`reassign_to`).
- **FR-F034-04:** `POST /api/v1/time-entries` records a native entry with `resource_id`, `row_id` (and its `project_sheet_id`), `entry_date` not later than today in the tenant time zone, `hours` in 0.25–24 with quarter-hour steps, and optional `note` (≤ 500); a user linked to the resource (`self`) or a `resource-admin` may create it; the daily total for the resource may not exceed 24 hours (`invalid` with `field_errors.hours`); the entry stores `source: native` and the resource's effective cost rate snapshotted into the typed columns `cost_rate`, `cost_currency`, and `cost_amount = hours × cost_rate`.
...
- **FR-F034-10:** The worker maintains `effort_summaries` for scopes `row`, `project`, and `resource_period` within 60 seconds of `time-entry.recorded.v1`, `time-entry.reconciled.v1`, `allocation.*.v1`, and `capacity.computed.v1`, recording `computed_at` on the summary and one `effort_summary_sources` row per source kind (`time_entry`, `reconciliation`, `allocation`, `capacity`) holding the `source_version` that produced it; reads serve from summaries and report `stale: true` when a queued source event carries a version above the stored row for its kind.
...
- Data access (decision section 2.1): `crates/persistence/src/workload/` holds `TimeEntryRepository` (owns `time_entries`), `WorkloadConflictRepository` (owns `workload_conflicts` and its child `workload_conflict_allocations`), and `EffortSummaryRepository` (owns `effort_summaries` and its child `effort_summary_sources`); no other class writes those tables. Named queries: `record_native_entry`, `daily_total_for_resource`, `find_native_collision`, `upsert_external_by_external_ref`, `list_entries_for_resource_range`, `list_pending_reconciliation`, `apply_reconciliation_decision`, `mark_superseded`, `sum_actual_hours_by_row`, `sum_actual_hours_by_resource`, `sum_pending_external_hours`; `upsert_open_conflict`, `replace_conflict_allocations`, `list_conflict_allocations`, `list_conflicts_page`, `resolve_periods_no_longer_over`, `list_conflicts_for_allocation`; `query_workload_page`, `get_row_effort`, `upsert_summary_with_sources`, `stored_source_version`, `mark_stale`, `rebuild_scope`. There is no generic query entry point.
- Every use case below depends on those repository traits and contains no SQL; `GET /api/v1/workload`, the conflicts list, the `conflict_detector` and `summary_builder` jobs, and the import and reconciliation paths all read and write through the repositories. A conflict upsert plus its allocation rows, an import batch, and a reconciliation decision set each run in one `UnitOfWork` that also carries the audit row and outbox enqueue; suggestion inputs come from the F012 `ScheduleResultRepository` and the F033 `AllocationRepository`/`ResourceRepository`, never from SQL written in this module.
- Domain entities in `crates/domain/src/workload/`: `TimeEntry { id, tenant_id, resource_id, row_id, project_sheet_id, entry_date, hours: Decimal, note, source: EntrySource, external_ref: Option<ExternalRef { source_system, external_id }>, reconciliation_state: ReconciliationState, reconciled_by, reconciled_at, resolution, reason, superseded_by, cost_rate: Decimal, cost_currency: CurrencyCode, cost_amount: Decimal, version, audit fields, deleted_at }`, `WorkloadRow { resource_id, period_start, period_end, available_hours, allocated_hours, actual_hours, utilization_pct, status }`, `WorkloadConflict { id, tenant_id, resource_id, period_start, period_end, available_hours, allocated_hours, over_hours, allocation_ids: Vec<AllocationId> (assembled by `WorkloadConflictRepository` from `workload_conflict_allocations`), status, detected_at, resolved_at, version }`, `ConflictSuggestion { kind: ShiftWithinFloat { allocation_id, float_days } | ReassignTo { resource_id, remaining_hours } }`, `EffortSummary { scope, scope_id, period_start, period_end, planned_hours, actual_hours, pending_external_hours, available_hours, planned_cost, actual_cost, computed_at, sources: Vec<SummarySource { kind, version }>, stale }`.
...
- Worker (`services/worker/src/workload/`): `conflict_detector.rs` consumes `capacity.computed.v1`; `summary_builder.rs` consumes `time-entry.recorded.v1`, `time-entry.reconciled.v1`, `allocation.*.v1`, `capacity.computed.v1` and rebuilds `effort_summaries` with its `effort_summary_sources` rows for the affected row, project, and resource periods; both jobs hold no SQL and call `EffortSummaryRepository` and `WorkloadConflictRepository`, and both are idempotent by `(scope_id, source_version)` compared against the stored row for the event's source kind.
...
- Normalized sets (decision section 2, no array columns): `workload_conflict_allocations(tenant_id uuid not null, conflict_id uuid not null references workload_conflicts(id) on delete cascade, allocation_id uuid not null references allocations(id) on delete restrict, primary key (conflict_id, allocation_id))` replaces `workload_conflicts.allocation_ids uuid[]`; the contribution row cannot outlive its conflict, so `conflict_id` cascades, while the F033 allocation it names is soft-deleted and restricted. `effort_summary_sources(tenant_id uuid not null, scope text not null, scope_id uuid not null, period_start date not null, source_kind text not null check (source_kind in ('time_entry','reconciliation','allocation','capacity')), source_version bigint not null, observed_at timestamptz not null, primary key (tenant_id, scope, scope_id, period_start, source_kind), foreign key (tenant_id, scope, scope_id, period_start) references effort_summaries(tenant_id, scope, scope_id, period_start) on delete cascade)` replaces `effort_summaries.source_versions jsonb`, which the jobs read by key to decide idempotency and staleness. `GET /api/v1/workload/conflicts` keeps `allocation_ids` as a JSON array and the effort and workload responses are unchanged: `WorkloadConflictRepository::replace_conflict_allocations` fans the set out to rows (`delete` of departed rows plus `insert ... on conflict do nothing`) inside the detector's `UnitOfWork` and `list_conflict_allocations` reassembles it on read, so no externally visible shape changes.
- `jsonb` audit (decision section 2): `time_entries.cost_snapshot jsonb` becomes typed `cost_rate`, `cost_currency`, `cost_amount` — `actual_cost` on `effort_summaries` and the effort response sum and filter on them, so they are queried structure, not an opaque payload. `effort_summaries.source_versions jsonb` becomes `effort_summary_sources` rows — the jobs look it up by source key and compare versions. No `jsonb` column remains in the `workload` module; audit before/after diffs and outbox payloads stay in the F003/F004 tables, where decision section 2 permits diffs and event payloads.
- Derived cache (decision section 2): `effort_summaries` and `effort_summary_sources` are a rebuildable cache, never the source of truth — `time_entries` and the F033 allocations are. They serve `EffortSummaryRepository::query_workload_page` behind `GET /api/v1/workload` and `get_row_effort` behind `GET /api/v1/rows/{id}/effort`, and are rebuilt by `services/worker/src/workload/summary_builder.rs` (`rebuild_summaries` for a full scope repair) from those base tables.
- Invariants: check `hours between 0.25 and 24 and (hours * 4) = floor(hours * 4)`; check `(source = 'external') = (source_system is not null and external_id is not null)`; check `(resolution is null) = (reconciled_at is null)`; check `cost_amount = round(hours * cost_rate, 2)`; unique partial index `time_entries_external_ref_idx on (tenant_id, source_system, external_id) where source = 'external' and deleted_at is null`; unique `(resource_id, period_start)` on `workload_conflicts`; `workload_conflict_allocations` primary key blocks a duplicate contributing allocation, and an open conflict must have at least one such row, enforced by `WorkloadConflictRepository::upsert_open_conflict` writing both in one statement pair; `effort_summaries` checks `num_nonnulls(row_id, project_sheet_id, resource_id) = 1` and that the non-null column matches `scope` (`row` → `row_id`, `project` → `project_sheet_id`, `resource_period` → `resource_id`), which gives every scope a declared foreign key while keeping `scope_id` as the generated identity used by the job idempotency key; `effort_summary_sources` holds at most one row per source kind per summary and its composite foreign key deletes with the summary; a statement trigger rejects `UPDATE` of `reconciled_by`, `reconciled_at`, `resolution`, or `reason` once set.
- Indexes: `time_entries(resource_id, entry_date) where deleted_at is null`, `time_entries(row_id, entry_date) where deleted_at is null`, `time_entries(project_sheet_id, entry_date)`, `time_entries(tenant_id, reconciliation_state) where reconciliation_state = 'pending'`, `effort_summaries(scope, scope_id, period_start)`, `effort_summary_sources(tenant_id, scope, scope_id, period_start)` for the staleness comparison, `workload_conflicts(tenant_id, status, period_start)`, `workload_conflicts(resource_id, status)`, `workload_conflict_allocations(conflict_id)` for the suggestion join and `workload_conflict_allocations(tenant_id, allocation_id)` for the reverse "which conflicts does this allocation cause" lookup after a `Shift` or `Reassign`.
...
- Retention/deletion: time entries soft-delete and are retained per tenant retention (F027); summaries are rebuildable and truncated on rollback; migration rollback drops the five tables, children (`effort_summary_sources`, `workload_conflict_allocations`) before parents, and the trigger.
...
- [ ] Database migration/constraint tests: hours check, cost amount check, external ref uniqueness, reconciliation immutability trigger, conflict uniqueness, duplicate `workload_conflict_allocations` row rejected, conflict delete cascades its allocation rows, `effort_summaries` scope/column check, one `effort_summary_sources` row per source kind, rollback ordering
...
- Migration adds `time_entries`, `effort_summaries`, `effort_summary_sources`, `workload_conflicts`, and `workload_conflict_allocations`; rollback drops them, children first. Feature is off by default behind `F034_FEATURE`.
