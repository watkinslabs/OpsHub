...
- **FR-F048-01:** `GET /api/v1/entitlements` returns one record per known module slug (`dynamic-views`, `workapps`, `data-shuttle`, `datamesh`, `bridge`, `calendar-app`, `pivots`, `assets`, `ai-assist`, `ai-insights`) with `state` (`none`, `trial`, `active`, `suspended`), `limits` object, `trial_ends_at`, `source` (`manual`, `plan`), and `version`; modules without a stored row are returned with `state: none` and `version: 0`.
