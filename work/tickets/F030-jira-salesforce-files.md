...
- **FR-F030-04:** `PATCH /api/v1/syncs/{id}` with `expected_version` updates `name`, `schedule`, `state`, `conflict_policy`, `deletion_policy`, and `source.filter`, returns the new `version`, and publishes `sync.updated.v1` with the changed keys; a stale `expected_version` returns `409 conflict` with the current version; activating a sync whose mapping set is empty returns `400 invalid`.
- **FR-F030-05:** `PUT /api/v1/syncs/{id}/mappings` replaces the whole mapping set in one transaction with rows `{ external_field, column_id, direction, transform, required, default_value? }`; the response returns the stored set and a new sync `version`; validation rejects a duplicate `external_field` or `column_id` per direction, a `required` mapping with no `default_value` on a nullable external field, and a transform whose output type does not match the column type, reporting `field_errors["mappings[3].transform"]`; a sync may hold at most 300 mappings.
- **FR-F030-06:** Transforms are pure functions from the fixed catalog `identity`, `trim`, `lower`, `upper`, `date_tz(tz)`, `datetime_format(pattern)`, `number_scale(factor)`, `value_map({external: opshub})`, `join(separator)`, `split(separator, index)`, `template(pattern)`, `lookup(sheet_id, key_column_id, value_column_id)`; an unknown name or a bad argument returns `400 invalid`; evaluation is sandboxed with no I/O other than `lookup`, is bounded to 5 ms per cell, and a transform failure marks that record `mapping_failed` without aborting the run.
...
- **FR-F030-10:** Every record failure is classified `transient` (HTTP 429, 502, 503, 504, timeout, connection reset), `permanent` (400, 401, 403, 404, 422 from the provider), or `mapping` (transform or type failure); `transient` is retried up to 5 times with exponential backoff 1 s, 2 s, 4 s, 8 s, 16 s plus ±20% jitter honoring `Retry-After`; `permanent` and `mapping` failures are written to the run without retry; a run ends `completed` with zero failures, `partial` when failures are under 10% of records read, and `failed` at 10% or more, which also sets the sync to `state: error` and pauses its schedule.
- **FR-F030-11:** `GET /api/v1/syncs/{id}/runs` returns cursor-paginated runs with `state` (`queued`, `running`, `completed`, `partial`, `failed`, `cancelled`), `trigger` (`schedule`, `manual`, `webhook`, `replay`), `started_at`, `finished_at`, `duration_ms`, counters `read`, `created`, `updated`, `skipped`, `conflicted`, `failed`, `cursor_before`, `cursor_after`, `error_class`, and up to 50 failed-record samples with `external_id`, `classification`, and provider error code; runs publish `sync-run.completed.v1` or `sync-run.failed.v1` and are retained 90 days under the F027 sweep.
- **FR-F030-12:** `POST /api/v1/sync-runs/{id}/replay` with `{ dry_run?, only_failed? }` starts a new run of `trigger: replay` from the source run's `cursor_before`, or over just that run's failed records when `only_failed` is true; `dry_run: true` returns `{ would_create, would_update, would_skip, would_conflict, samples[] }` without writing anything; replay is idempotent through `sync_record_links` keyed `(sync_id, external_id)` compared on `external_version`, so re-applying an already-applied record counts as `skipped`.
- **FR-F030-13:** When a record's OpsHub row and its external counterpart both changed since `cursor_value` — detected by comparing `sync_record_links.opshub_updated_at` and `external_updated_at` against the stored values — the run writes a `sync_conflicts` row with `field_diffs: [{ column_id, external_field, opshub_value, external_value }]`, both timestamps, and `state: open`, and publishes `sync-conflict.detected.v1`; `conflict_policy` `manual` (default for F030) leaves both sides untouched, while `opshub_wins`, `external_wins`, and `newest_wins` apply the winner immediately and store the conflict `state: auto_resolved` with `resolution` for audit.
- **FR-F030-14:** `GET /api/v1/syncs/{id}/conflicts` lists conflicts filtered by `state` with both values per field, and `POST /api/v1/sync-conflicts/{id}/resolve` with `{ resolution: keep_opshub|keep_external|merge, field_values?, expected_version }` writes the chosen values to both sides, sets `state: resolved` with `resolved_by` and `resolved_at`, and publishes `sync-conflict.resolved.v1`; `merge` requires `field_values` covering every conflicting field (`400 invalid` otherwise) and a conflict already `resolved` or `auto_resolved` returns `409 conflict`.
...
- **NFR-F030-02 Security/privacy:** F030 stores no credentials — access tokens come from the F029 vault per call and database DSNs from the secret manager, never from `syncs` or `sync_mappings`; the database connector is read-only with parameterized statements and an object allowlist; transforms have no network or filesystem access; run logs store field values only as SHA-256 digests unless the sync sets `debug_payloads: true`, which retains raw samples for 7 days and writes an audit event; provider webhooks are signature-verified before enqueueing; cross-tenant reads and writes are tested negative on every route.
...
- Migration `*_connectors_*.sql` creates `syncs(id uuid pk, tenant_id, connection_id uuid not null, name text not null, connector text not null, kind text not null, source_object text not null, source_filter text, target_sheet_id uuid not null, direction text not null, schedule text not null default 'every_15m', conflict_policy text not null default 'manual', deletion_policy text not null default 'ignore', deletion_column_id uuid, debug_payloads boolean not null default false, state text not null default 'paused', last_run_id uuid, version bigint not null default 1, audit fields, deleted_at)`, `sync_mappings(id uuid pk, tenant_id, sync_id uuid not null references syncs(id) on delete cascade, external_field text not null, column_id uuid not null, direction text not null, transform jsonb not null default '{"name":"identity"}', required boolean not null default false, default_value jsonb, position int not null)`, `sync_runs(id uuid pk, tenant_id, sync_id uuid not null references syncs(id) on delete cascade, trigger text not null, state text not null, counters jsonb not null default '{}', cursor_before text, cursor_after text, error_class text, api_version text, failed_samples jsonb, started_at timestamptz, finished_at timestamptz, duration_ms int)`, `sync_cursors(sync_id uuid not null references syncs(id) on delete cascade, direction text not null, cursor_kind text not null, cursor_value text, high_water_mark timestamptz, checkpoint_record_id text, updated_at timestamptz not null, primary key (sync_id, direction))`, `sync_conflicts(id uuid pk, tenant_id, sync_id uuid not null references syncs(id) on delete cascade, external_id text not null, row_id uuid, field_diffs jsonb not null, opshub_updated_at timestamptz, external_updated_at timestamptz, state text not null default 'open', resolution text, resolved_by uuid, resolved_at timestamptz, version bigint not null default 1, detected_at timestamptz not null)`, `sync_record_links(sync_id uuid not null references syncs(id) on delete cascade, external_id text not null, tenant_id, row_id uuid not null, external_version text, external_updated_at timestamptz, opshub_updated_at timestamptz, deleted_external boolean not null default false, primary key (sync_id, external_id))`.
- Invariants: unique `syncs(connection_id, source_object, target_sheet_id, direction) where deleted_at is null and state <> 'paused'`; unique `sync_mappings(sync_id, external_field, direction)` and `sync_mappings(sync_id, column_id, direction)`; check `sync_mappings` count per sync ≤ 300 enforced in `replace_mappings` inside the transaction plus a statement trigger; unique `sync_record_links(sync_id, row_id)`; check `syncs.deletion_column_id is not null or deletion_policy <> 'mark_deleted'`; check `sync_runs.state in ('queued','running','completed','partial','failed','cancelled')`.
- Indexes: `syncs(tenant_id, state, connector)`, `syncs(connection_id)`, `sync_runs(sync_id, started_at desc)`, `sync_runs(tenant_id, state, started_at desc)`, `sync_conflicts(sync_id, state, detected_at desc)`, `sync_conflicts(tenant_id, state)`, `sync_record_links(sync_id, row_id)`, `sync_mappings(sync_id, position)`.
...
- Retention/deletion: `sync_runs` older than 90 days deleted nightly; `failed_samples` value payloads expire after 7 days; resolved conflicts kept 180 days; deleting a sync cascades mappings, runs, cursors, conflicts, and links; rollback drops the six tables and their indexes.
...
- [ ] Database migration/constraint tests: unique active sync tuple, mapping uniqueness and cap, cascade delete, `deletion_column_id` check, run state check, rollback
...
Scenario: Both sides changed and the conflict waits for a person
  ...
  Then neither side is written, a sync_conflicts row records both values per field, and sync-conflict.detected.v1 is published
...
- Conflicts with: none — F030 owns `crates/domain/src/connectors/**`, `services/api/src/connectors/**`, `services/worker/src/connectors/**`, `apps/web/src/features/connectors/**`, and `*_connectors_*.sql`, all disjoint from F029's `integrations` module
...
- Migration adds `syncs`, `sync_mappings`, `sync_runs`, `sync_cursors`, `sync_conflicts`, and `sync_record_links`; rollback drops them and leaves F029 connections intact. Feature is off by default behind `F030_FEATURE`.
