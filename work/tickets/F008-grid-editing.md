...
- **FR-F008-05:** Every `PATCH cells`, `cells/bulk`, and `rows/bulk` call creates one `edit_batches` row for the actor with the inverse edits; each actor keeps at most 50 undoable batches per sheet and the oldest batch is trimmed when a 51st is created.
- **FR-F008-06:** `POST /api/v1/sheets/{sheet_id}/undo` applies the inverse of the actor's most recent not-undone batch only when every affected cell still carries the row version recorded in the batch; a mismatch returns `conflict` listing the changed cells and leaves the batch on the stack; success sets `undone_at`, publishes `edit.undone.v1`, and returns the restored cells.
...
- **FR-F008-10:** The `layout` object `{ column_widths, column_order, hidden_columns, frozen_column_count }` sent as an optional top-level field of `PATCH /api/v1/sheets/{sheet_id}/cells` upserts `sheet_user_layouts` for the actor in the same request (an `edits` array may be empty when only `layout` is sent), and `GET /api/v1/sheets/{sheet_id}/changes` returns the actor's current `layout`; the primary column cannot be hidden and `frozen_column_count` is 0 to 5.
...
- **FR-F008-14:** Column resize, reorder, hide, and freeze are core grid behaviour at every product tier, never a paid or licence-gated capability, and are implemented on the F062 Community grid wrapper against this feature's `layout` field rather than a vendor grid's paid tier. They are applied immediately in the grid and persisted through the `layout` field within 1 s of the last change, so the layout is restored for that user on the next visit and is never shared with other users.
...
- Domain entities in `crates/domain/src/grid/`: `CellEdit { row_id, column_id, value: Value, expected_row_version }`, `CellEditResult { row_id, column_id, outcome: Applied { row_version, display } | Invalid { code } | Conflict { current_row_version } }`, `EditBatch { id, tenant_id, sheet_id, actor_id, kind: Patch|BulkCells|BulkRows|Undo|Redo, cell_count, inverse: Vec<InverseCell>, undone_at, redone_at, version }`, `CellHistoryEntry { row_id, column_id, version, previous_raw, new_raw, actor_id, batch_id, occurred_at }`, `UserLayout { column_widths: Map<ColumnId, u16>, column_order: Vec<ColumnId>, hidden_columns: Vec<ColumnId>, frozen_column_count: u8 }`, `ChangeFeedEntry`.
...
- Migration `*_grid_*.sql` creates `cell_history(id uuid pk, tenant_id uuid not null, row_id uuid not null, column_id uuid not null, version bigint not null, previous_raw jsonb, new_raw jsonb, actor_id uuid not null, batch_id uuid not null, occurred_at timestamptz not null)`, `edit_batches(id uuid pk, tenant_id, sheet_id, actor_id, kind text check (kind in ('patch','bulk_cells','bulk_rows','undo','redo')), cell_count int not null, inverse jsonb not null, undone_at timestamptz, redone_at timestamptz, version bigint not null default 1, created_at, created_by, updated_at, updated_by)`, `sheet_user_layouts(tenant_id, sheet_id, user_id, column_widths jsonb not null default '{}', column_order jsonb not null default '[]', hidden_columns jsonb not null default '[]', frozen_column_count smallint not null default 0 check (frozen_column_count between 0 and 5), version bigint not null default 1, updated_at, primary key (tenant_id, sheet_id, user_id))`, and adds `sheets.change_version bigint not null default 0` through an additive `alter table`.
- Invariants: `cell_history(row_id, column_id, version)` unique; `edit_batches` foreign key to `sheets` with `on delete restrict`; at most 50 non-trimmed batches per `(sheet_id, actor_id)` enforced by the service, verified by a test; `sheets.change_version` increments once per applied cell inside the write transaction.
- Indexes: `cell_history(row_id, column_id, occurred_at desc)`, `cell_history(tenant_id, row_id)`, `edit_batches(sheet_id, actor_id, created_at desc) where undone_at is null`, `edit_batches(sheet_id, actor_id, undone_at desc) where undone_at is not null and redone_at is null`, `cells(sheet_change_version)` via a new column `cells.change_version bigint` for the feed query.
...
- Migration adds `cell_history`, `edit_batches`, `sheet_user_layouts`, and change-version columns on `sheets` and `cells`; rollback drops them. Feature is off by default behind `F008_FEATURE`.
