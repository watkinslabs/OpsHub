...
- **FR-F008-14:** Column resize, reorder, hide, and freeze are applied immediately in the grid and persisted through the `layout` field within 1 s of the last change, so the layout is restored for that user on the next visit and is never shared with other users.
