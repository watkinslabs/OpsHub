...
conflicts_with: []
parallel_safe: true
owned_paths: [crates/domain/src/i18n/**, crates/persistence/src/i18n/**, services/api/src/i18n/**, apps/web/src/features/i18n/**, services/api/migrations/*_i18n_*.sql, testing/features/F049/**]
...
- **FR-F049-08:** `GET /api/v1/messages/{locale}` returns the ICU MessageFormat catalog as one document `{ locale, version, fallback: "en-US", messages: { key: pattern } }` assembled by `MessageCatalogRepository::load_catalog` from the `message_catalog_entries` rows of the latest version in a single query, with the strong `ETag` taken unchanged from `message_catalogs.etag` and `Cache-Control: public, max-age=86400, immutable`; `If-None-Match` matching the current version returns `304`; an unsupported locale returns `404 not_found`.
- **FR-F049-09:** Any catalog key missing in a non-`en-US` locale falls back to the `en-US` pattern by a keyed lookup (`find_pattern('en-US', latest_version, key)` against `message_catalog_entries`, served by the `message_key` index) and increments the `i18n_missing_key_total{locale,key}` counter once per key per process; the web client applies the same per-key fallback from the bundled `en-US` catalog.
- **FR-F049-10:** Plural and select forms follow ICU rules per locale, validated per entry row at build time: `{count, plural, one {# row} other {# rows}}` produces `1 row` and `2 rows` in `en-US`, and the `ja-JP` catalog with only `other` produces `2 行` without a missing-form error.
...
- **FR-F049-13:** The pseudo-locale `en-XA` wraps every `en-US` pattern as `[!!! Ẽẽẽ !!!]` with 40% padding and writes the wrapped patterns as `en-XA` entry rows under their own header version, so untranslated strings and layout overflow are visible in tests; it is never listed to tenants in production.
...
- Persistence (`crates/persistence/src/i18n/`): `TenantLocaleRepository` owns `tenant_locales`; `UserLocaleRepository` owns `user_locales`; `MessageCatalogRepository` owns `message_catalogs` and `message_catalog_entries`. Each implements the shared `Repository` contract (`get`, `list` with cursor pagination, `insert`, `update` under an expected version, `soft_delete`, `restore`, `purge`) and adds named queries `get_effective(tenant_id, user_id)`, `upsert_tenant_locale`, `upsert_user_override`, `clear_user_override`, `latest_version(locale)`, `load_catalog(locale, version)`, `find_pattern(locale, version, key)`, `replace_catalog(locale, version, entries)`; the tenant predicate, soft-delete filter, version check, audit row, and outbox enqueue come from the base contract. The catalog build job replaces a locale's header row and all of its entry rows through `replace_catalog` in one `UnitOfWork` that owns the transaction, so a reader never sees a partial version. The ICU formatter and the fallback resolver are pure functions over loaded values and touch no database. Per decision 2.1 the use cases above depend on these repository traits and contain no SQL: no SQL string, `sqlx::query*` call, or connection exists in `crates/domain/src/i18n` or `services/api/src/i18n`.
...
- Invariants: check constraints `locale in ('en-US','en-GB','de-DE','fr-FR','es-ES','pt-BR','ja-JP','en-XA')` on both settings tables and on `message_catalogs`, `hour_cycle in ('h12','h23')`, `first_day_of_week in ('monday','sunday','saturday')`; `message_catalog_entries.message_key` matches `^[a-z0-9]+(\.[a-z0-9-]+)+$` and `pattern` is non-empty; `message_catalogs.entry_count` must equal the number of `message_catalog_entries` rows loaded for that `(locale, version)` — the check that replaced `jsonb_typeof(messages) = 'object'` — and `replace_catalog` sets it in the same transaction that inserts the entry rows, so a header never claims a count it does not have; one row per tenant in `tenant_locales` created by the tenant creation transaction of F002 through a trigger in this migration.
- Indexes: `user_locales(user_id)`, `message_catalogs(locale, version desc)`, `message_catalog_entries(message_key)` for the cross-locale `en-US` fallback lookup and the per-key coverage queries (the primary key already serves `load_catalog`'s single-query fetch of one version); all database text columns use `COLLATE "C"` for identifiers and ICU collation `und-u-ks-level2` for user-visible sort in list queries added by later features.
- Behaviour preserved by the split: `GET /api/v1/messages/{locale}` still returns one `{ locale, version, fallback, messages: { key: pattern } }` document, assembled by `MessageCatalogRepository::load_catalog` from the entry rows in one query; the strong `ETag` still comes from `message_catalogs.etag` and is unchanged; `Cache-Control` and the `304` path are unchanged; the FR-F049-09 fallback becomes a keyed lookup against the `en-US` entry rows instead of a JSON key probe.
- Audit events: `tenant-locale.update`, `user-locale.update` with field-level diffs.
- Retention/deletion: settings rows follow the owning tenant/user lifecycle; `message_catalogs` and `message_catalog_entries` rows are static content refreshed by the catalog build job and never soft-deleted; migration rollback drops the trigger and the four tables `tenant_locales`, `user_locales`, `message_catalog_entries`, and `message_catalogs`.
...
- [ ] Database migration/constraint tests: check constraints, tenant trigger, `entry_count` matches the loaded entry rows, entry foreign key cascades on header delete, rollback
...
- Fixture/seed factory: `testing/fixtures/i18n.rs` builds tenant A (`de-DE`, `Europe/Berlin`), tenant B (`en-US`, `UTC`), an admin, a user with a `pt-BR`/`America/Sao_Paulo` override, a user without override, and a foreign-tenant actor; catalog headers for all eight locales with 2,000 `message_catalog_entries` rows each
...
Scenario: Catalog fallback and telemetry
  Given the ja-JP catalog has no entry row for key "sheet.restore.confirm"
...
- Migration adds `tenant_locales`, `user_locales`, `message_catalogs`, and `message_catalog_entries`; rollback drops all four. Feature is off by default behind `F049_FEATURE`.
