...
- Owner: platform
- Branch: `t135-balancing-ui`
- Decision references: `docs/architecture-decisions.md` sections 2, 2.1, 3, 4; `docs/capability-contracts.md` row F034
...
- Data access: the client owns no schema knowledge — it consumes the T133/T134 DTOs, in which a conflict still carries an `allocation_ids` array and the effort response still carries `by_resource[]`, so the normalization behind them (`workload_conflict_allocations`, `effort_summary_sources`, typed cost columns) changes nothing on this surface; component and E2E fixtures are seeded through `crates/persistence/src/workload/` repositories in `testing/fixtures/workload.rs`, never by SQL in a spec file (decision section 2.1).
