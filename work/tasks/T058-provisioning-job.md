...
parent_story: S029
depends_on: [T057]
owned_paths: [crates/domain/src/templates/**, services/api/src/templates/**, services/worker/src/templates/**, apps/web/src/features/templates/**, testing/features/F015/api/**, testing/features/F015/frontend/**, testing/features/F015/performance/**]
...
- Output/behavior: `POST /api/v1/project-templates/{id}/provision` requires `portfolio-admin` plus `workspace-admin` on the target, rejects draft versions, inserts the run as `queued`, enqueues the job, and returns `202 ProvisioningRunResponse` under 2 s; the worker runs steps `sheets`, `columns`, `rows`, `schedule_settings`, `dependencies`, `views`, `forms`, `roles` through the F006, F007, F011, F012, F013, F014 domain services, idempotent by `(run_id, step, item_key)`, resolving default-row dates with `add_working_days(start_date, offset)`; unknown modules are recorded as `skipped`; three retries with exponential backoff, then `rollback_run` soft-deletes created objects in reverse and emits `provisioning.failed.v1`; completion emits `project.provisioned.v1`; UI polls the run every 2 s, announces step progress, and offers `Open project` or `Retry provisioning`; telemetry `template_provision_requested`, `provisioning_completed`, `provisioning_failed`.
