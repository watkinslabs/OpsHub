...
- Owned paths: `crates/domain/src/auth/{factor.rs, totp.rs, webauthn.rs, service_mfa.rs}`, `services/api/src/auth/handlers_mfa.rs`, `crates/persistence/src/auth/mfa_factor_repository.rs` (the only file here that may contain SQL), `apps/web/src/features/auth/{LoginPage.tsx, TenantSlugForm.tsx, CallbackPage.tsx, MfaInterstitial.tsx, SecuritySettingsPage.tsx, SessionsList.tsx, RevokeSessionDialog.tsx, MfaEnrollTotpDialog.tsx, MfaWebAuthnButton.tsx, FactorsList.tsx, AuthProvider.tsx, api.ts, hooks.ts, routes.ts}`
- Contract/input: `TotpVerifyRequest { factor_id, code: 6 digits }`, `WebAuthnRegisterRequest` (first call empty, second call `{ challenge_id, attestation }`), `WebAuthnAssertRequest { challenge_id, assertion }`; RP id and origin from `RuntimeConfig`; challenges stored 5 minutes in `mfa_factors` pending rows written by `MfaFactorRepository`, which owns every query against that table.
- Output/behavior: routes `POST /api/v1/mfa/totp/enroll`, `POST /api/v1/mfa/totp/verify`, `POST /api/v1/mfa/webauthn/register`, `POST /api/v1/mfa/webauthn/assert`, `DELETE /api/v1/mfa/factors/{id}`; TOTP RFC 6238 SHA-1 30 s window ±1 step, secret envelope-encrypted with `SecretCipher`; WebAuthn verifies origin, RP id hash, user presence, and strictly increasing `sign_count`; 5-factor cap and the last-factor rule read from `SecurityPolicyRepository`; sets `sessions.mfa_verified_at` through `SessionRepository`; `service_mfa.rs`, `handlers_mfa.rs`, the React pages, and the tests contain no SQL (decision 2.1); events `mfa.enrolled.v1`, `mfa.removed.v1`; pages implement the flows and states from ticket section 3 including the QR code with copyable secret and the `AuthProvider` refresh-once retry.
