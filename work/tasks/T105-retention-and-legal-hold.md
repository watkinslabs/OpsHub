...
parent_story: S053
depends_on: [S053]
owned_paths: [services/api/migrations/*_compliance_*.sql, crates/domain/src/compliance/**, crates/persistence/src/compliance/**, services/api/src/compliance/**, services/worker/src/compliance/**, apps/web/src/features/compliance/**, testing/features/F027/api/**, testing/features/F027/database/**, testing/features/F027/frontend/**]
...
- Output/behavior: routes `GET /api/v1/compliance/retention-policies`, `PUT /api/v1/compliance/retention-policies/{id}`, `POST /api/v1/compliance/legal-holds`, `DELETE /api/v1/compliance/legal-holds/{id}`; policies seeded per tenant on first read with defaults (30 days soft delete, purge `null`); `audit_events` policy floor 365; `hold.rs` exposes `HoldScope::matches(record_ref)` and `is_held` used by the sweep and by T106; release checks `security_policies.two_person_hold_release`; sweep soft-deletes only `auto_soft_delete` kinds, marks `purge_eligible`, skips held records, runs under per-tenant quota 1 with 1,000-row batches; events `retention-policy.updated.v1`, `legal-hold.applied.v1` (`action: applied|released`); audit `retention-policy.update`, `legal-hold.apply`, `legal-hold.release`; DDL for `retention_policies`, `legal_holds`, `tenant_exports`, `tenant_export_kinds`, `purge_requests`, `purge_request_kinds`, `purge_batches`, `access_reviews`, `access_review_principals`, `access_review_decisions`, the `record_kind`, `scope_kind`, `status`, `format`, `flag_reason`, `decision`, and `outcome` `check` constraints, the actor foreign keys to `users(id) on delete restrict`, the `on delete cascade` parent keys on every child table, the purge status trigger, and the indexes from ticket section 4; the down migration drops children before parents.
- Data access: `policy.rs`, `hold.rs`, and `service.rs` hold no SQL. `RetentionPolicyRepository` (owns `retention_policies`) exposes `seed_defaults_for_tenant`, `list_policies`, `find_policy_by_kind`, and `put_policy_under_version`; `LegalHoldRepository` (owns `legal_holds`) exposes `list_active_holds`, `find_holds_covering`, `count_held_by_kind`, and `mark_released`. `retention_sweep.rs` reads through both and soft-deletes targets by calling `soft_delete` on the owning F006/F045/F017/F016/F019 repositories through the `RetentionTarget` port, never by issuing its own statement; policy seeding and the sweep's per-batch writes run in one `UnitOfWork` (decision section 2.1).
