...
- Owner: platform
- Branch: `t212-conflict-tests`
- Decision references: `docs/architecture-decisions.md` sections 2, 2.1, 7, 9; `docs/capability-contracts.md` row F053
...
- Owned paths: `testing/features/F053/database/constraint_tests.rs`, `testing/features/F053/e2e/datamesh.spec.ts`, `testing/features/F053/accessibility/datamesh.a11y.spec.ts`, `testing/features/F053/performance/{preview_bench.rs, sync_bench.rs, conflicts_bench.rs}`, `testing/features/F053/api/concurrency_tests.rs`, `testing/features/F053/requirements/cases.md` (final traceability), `testing/features/F053/README.md`
...
- Data access: no test opens a connection or issues SQL of its own; every fixture write, seed, and assertion goes through the `crates/persistence/src/datamesh/` repositories (`MappingRepository`, `MatchRepository`, `MeshRunRepository`, `ConflictRepository`) and the F006/F008 repositories, and the concurrency suite drives runs through `MeshRunRepository::claim_run` rather than raw `for update` statements (decision section 2.1). `constraint_tests.rs` asserts the normalized shape: `datamesh_mapping_match_keys` rejects an `ordinal` outside 1–3 and a duplicate column pair, `datamesh_mapping_field_maps` rejects a second row for one `target_column_id` and a bidirectional row carrying an expression, the partial unique index rejects a target column already mapped by another enabled mapping, a `scheduled` mapping without `cron_expression` is rejected, deleting a mapped column is restricted by the child foreign keys, purging a mapping cascades its match-key, field-map, and match rows while leaving runs and conflicts restricted, and `datamesh_conflicts.source_value`/`target_value` are the only remaining `jsonb` columns in the module.
...
- Failing test first: `testing/features/F053/e2e/datamesh.spec.ts::create_mapping_preview_sync_resolve`, `::on_change_sync_after_source_edit`, `::provenance_link_visible_in_target_grid`, `::viewer_sees_read_only_tabs`, `::flag_off_hides_module`; `testing/features/F053/api/concurrency_tests.rs::two_workers_process_run_once`, `::edit_during_run_becomes_conflict_not_overwrite`, `::bidirectional_loop_terminates`; `testing/features/F053/database/constraint_tests.rs::match_key_ordinal_out_of_range_rejected`, `::duplicate_match_key_column_pair_rejected`, `::field_map_row_unique_per_target_column`, `::target_column_owned_by_enabled_mapping_rejected`, `::bidirectional_field_map_with_expression_rejected`, `::scheduled_mapping_requires_cron_expression`, `::mapped_column_delete_restricted`, `::mapping_purge_cascades_child_rows_and_restricts_runs`, `::only_conflict_value_columns_are_jsonb`; `testing/features/F053/accessibility/datamesh.a11y.spec.ts::pages_have_no_serious_axe_violations`; `testing/features/F053/performance/sync_bench.rs::sync_10k_changed_rows_under_2_minutes`
