...
- Dependencies: T093 spec model, `ChartDefinitionRepository` and query route; T094 time-series route; F008 `cell_history` through its own repository; F022 `KpiCard` wrapped by `KpiWidget`; F023 renderer registry and widget config panel; F049 formatter.
