...
- Contract/input: `SyncRequest { connection_id, name, kind, source: { object, filter? }, target: { sheet_id }, direction, schedule, conflict_policy?, deletion_policy?, deletion_column_id?, mappings }`; `PATCH` body `{ name?, schedule?, state?, conflict_policy?, deletion_policy?, source_filter?, reset_cursor_to? }`; list query `{ cursor?, limit?, connection_id?, connector?, kind?, state? }`; every mutation carries `Idempotency-Key` and `If-Match` (FR-F030-04).
