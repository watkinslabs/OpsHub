...
- Contract/input: `issue_token(publication) -> (plaintext, PublicationToken)` with 32 random bytes, base64url plaintext, SHA-256 `token_hash`, and the scope persisted as the typed columns `tenant_id`, `publication_id`, `scope_target_kind`, `scope_target_id`, `read_only` while `TokenScope` still serialises as `{ tenant_id, publication_id, target, read_only: true }`; `POST /api/v1/publications/{id}/rotate-token`; `GET /public/publications/{token}`; `resolve_token(plaintext) -> Result<ResolvedScope, PublishError>`.
...
- Data access: `PublicationTokenRepository` in `crates/persistence/src/publishing/token_repository.rs` owns `publication_tokens` and is the only class that writes it, exposing `find_by_token_hash`, `insert_rotated_token`, `supersede_with_grace`, `revoke_all_for_publication`, and `purge_superseded_before`; `resolve.rs` and the unauthenticated `handlers_public.rs` resolve a presented token through `find_by_token_hash` and hold no SQL, no `sqlx::query*` call, and no pool handle; rotation inserts the new token, supersedes the old one, and writes the audit and outbox rows in one `UnitOfWork` with `PublicationRepository`, and the public render reads `source_versions` through `PublicationRepository::list_snapshot_sources` (decision section 2.1).
...
- Failing test first: `testing/features/F059/api/token_tests.rs::token_stored_only_as_hash`, `::token_not_returned_after_creation`, `::rotate_token_grace_then_404`, `::revoked_token_404_within_5s`, `::expired_token_404`, `::public_render_returns_generated_at_and_stale_header`, `::public_render_stale_after_refresh_failure`, `::public_render_error_state_when_target_deleted`, `::token_scope_columns_reject_foreign_target`, `::public_render_source_versions_match_snapshot_source_rows`
