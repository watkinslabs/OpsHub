...
- Data access: this task adds no server code and holds no SQL; editors emit one op per edited cell and one op per submitted form with its field values, which T231 hands to `SyncBatchRepository` as `mobile_sync_batch_ops` and `mobile_sync_op_values` rows, and the online path calls the generated `GridApi`/`FormsApi` clients whose request and response shapes are unchanged by that normalization (decision section 2.1).
