...
- Contract/input: engine input `RunPlan { sync, mappings, cursor, page_size: 200, trigger }`; `POST /api/v1/sync-conflicts/{id}/resolve` body `{ resolution: keep_opshub|keep_external|merge, field_values? }` under `If-Match` on the parent sync; `GET /api/v1/syncs/{id}/conflicts` query `{ state?, cursor?, limit? }`; secret-manager key `connectors/database/{sync_id}/dsn`.
