...
- Output/behavior: the redactor replaces email, E.164 phone, and 13–19 digit card matches with `<redacted:email>`, `<redacted:phone>`, `<redacted:card>` and under `strict` drops every column whose F007 metadata sets `sensitive: true`; redaction runs on the serialized envelope and its BLAKE3 hash is returned as `envelope_hash` so no envelope content is persisted. `compile` maps the completion into an F021 `ReportDefinition` restricted to `scope.readable_sheets`, rejects any alias referencing a denied sheet, runs the F021 definition validator (sources 1–20, joins forming a tree rooted at `sources[0]`, filter depth ≤ 4 and ≤ 50 predicates, `group_by` ≤ 3, `aggregates` ≤ 20, `calculated_fields` ≤ 25 parsed with F035), and on failure returns `CompileError { field_errors }` so T153 can request exactly one regeneration; a second failure surfaces `AiError::UncompilablePlan → 502 unavailable` and the rejected plan is stored on the `ai_requests` row by `AiRequestRepository::store_rejected_plan`. Each candidate sheet is recorded by `AiRequestRepository::replace_request_sources` as one `ai_request_sources` row — `included` with an alias, or excluded with `denied`, `not_found`, or `over_limit` — and the response builds its `sources` and `excluded_sources` arrays from those rows. `execute` runs a stored plan through the F021 `ReportRepository` ad-hoc read path under the caller's live permissions and returns rows plus `meta { computed_at, duration_ms, restricted_sources, hidden_columns, truncated }`, recording the run through `AiQueryExecutionRepository::append_execution` with one `ai_query_execution_restricted_sources` row per suppressed sheet and one `ai_query_execution_hidden_columns` row per suppressed column in the same `UnitOfWork`, while `meta` keeps its array shape. `plan_hash` is the BLAKE3 hash of the canonicalized plan and a mismatch on execute returns `409 conflict` with `current_plan_hash`.
- Data access: this task owns no table and writes no SQL — `retrieval/*` and `query/*` read schema cards, column metadata, and sample rows through the F005/F007 `SheetRepository`, `ColumnRepository`, and `RowRepository` named queries under the caller's context, execute plans through the F021 `ReportRepository` read query, and persist request sources, rejected plans, and executions through the `crates/persistence/src/ai-assist/` repositories T153 creates. Permission-filtered retrieval never assembles a SQL string, and no model output reaches the database as SQL: the compiled artifact is an F021 `ReportDefinition` validated before execution (decision section 2.1).
...
- [ ] Owned-path check passes and `crates/domain/src/ai-assist/provider/**` and `crates/persistence/src/ai-assist/**` are left to T153
- [ ] `cargo xtask check-persistence` passes: neither `retrieval/*` nor `query/*` contains a `sqlx::query*` call, a connection, or a composed SQL string
