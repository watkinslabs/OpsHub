...
Create the four `dataio` tables, the outbox-driven search indexer, and the ACL-filtered `GET /api/v1/search` route so tenant-scoped full-text search works before any import or export code exists.
...
- Output/behavior: migration creates `search_documents`, `import_jobs`, `import_rows`, `export_jobs` with the check constraints, primary keys, GIN indexes on `body` and `body_simple`, and the secondary indexes from ticket section 4; the indexer builds `title` and `body` per kind (row body is the concatenation of text-like cell display values, comment body is a 200-char prefix plus author, attachment body is filename plus MIME plus size), upserts with `on conflict ... where source_version < excluded.source_version`, deletes on soft delete, and emits `search.indexed.v1`; the query joins `english` and `simple` `to_tsquery` with prefix matching on the last term, ranks by `ts_rank_cd`, snippets with `ts_headline`, prefilters by `acl_snapshot`, then calls the F003 policy engine per hit and drops denied hits before paging; response `SearchResponse { hits, next_cursor }`; foreign-tenant filters return an empty page.
...
- Large-table note: `search_documents` grows to millions of rows; the GIN indexes are created with `fastupdate = off` and the consumer batches upserts in groups of 500.

## TDD

- Failing test first: `testing/features/F010/database/migration_tests.rs::dataio_tables_exist_with_constraints`, `::search_body_gin_index_used`, `::stale_source_version_upsert_is_noop`, `::rollback_drops_tables`; `testing/features/F010/api/search_tests.rs::search_returns_ranked_hits_with_snippets`, `::search_omits_unreadable_sheet_rows`, `::search_cross_tenant_returns_empty`, `::search_empty_query_invalid`, `::indexer_upserts_and_ignores_stale_version`, `::indexer_removes_soft_deleted_source`, `::indexer_never_reads_file_bodies`
