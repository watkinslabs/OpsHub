...
parent_story: S073
depends_on: [S073]
owned_paths: [services/api/migrations/*_notifications_*.sql, crates/domain/src/notifications/**, crates/persistence/src/notifications/**, services/api/src/notifications/**, services/worker/src/notifications/**, testing/features/F037/api/**, testing/features/F037/database/**]
...
- Output/behavior: routes `GET /api/v1/notifications`, `POST /api/v1/notifications/{id}/read`, `POST /api/v1/notifications/read-all`; DTOs `NotificationResponse`, `InboxPage { items, next_cursor, unread_count }`, `ReadAllResponse { updated_count }`; `service.rs` calls `NotificationRepository::create_if_absent(tenant_id, source_event_id, recipient_id, notification)` inside the caller's `UnitOfWork`, rejects unknown categories with `NotificationError::InvalidCategory`, and returns the existing row for a `dedupe_key` repeated within 24 hours — the advisory lock over that window is taken inside `create_if_absent` and the same partial unique index still enforces it, so no caller writes an `ON CONFLICT`; `router.rs` calls `NotificationPreferenceRepository::resolve_effective_preferences(tenant_id, user_id)` for the user's `notification_channel_preferences` rows, then the tenant-default row's rows, then built-in defaults (in-app on for all categories, email on for `approval`, `assignment`, `mention`, `update_request`, push off) and inserts one `notification_deliveries` row per channel in the same `UnitOfWork`, `queued` when the row is `enabled` and `suppressed` with `reason = preference` when not; `event_consumer.rs` maps the subjects above to categories and deduplicates on `source_event_id` through `create_if_absent` and the unique `(tenant_id, source_event_id, recipient_id)`; publishes `notification.created.v1`; inbox handlers call `page_for_recipient`, `unread_count`, `mark_read`, and `mark_all_read` and hold no SQL string, `sqlx::query*` call, or connection; DDL for `notifications`, `notification_deliveries`, and `push_subscriptions` with the category, channel, and status checks and the indexes `notifications(tenant_id, recipient_id, created_at desc)`, `notifications(tenant_id, recipient_id) where read_at is null`, and `notification_deliveries(status, next_attempt_at) where status in ('queued','digested')`; inbox is strictly `self` scoped and cross-tenant IDs return `404 not_found`.
