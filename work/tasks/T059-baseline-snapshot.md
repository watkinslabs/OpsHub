...
parent_story: S030
depends_on: [T058]
owned_paths: [crates/domain/src/templates/**, crates/persistence/src/templates/**, services/api/src/templates/**, testing/features/F015/api/**, testing/features/F015/database/**, testing/features/F015/performance/**]
...
- Owned paths: `crates/domain/src/templates/{baseline.rs, variance.rs, service_baseline.rs}`, `crates/persistence/src/templates/{mod.rs, baseline_repository.rs}`, `services/api/src/templates/{handlers_baseline.rs, handlers_variance.rs}`
...
- Output/behavior: `POST /api/v1/sheets/{sheet_id}/baselines` requires `portfolio-admin`, resolves measure names to numeric or duration columns from the sheet schedule settings, and calls `BaselineRepository::capture_baseline_rows(baseline_id, rows)`, which writes the header, one `baseline_measures` row per selected measure, the `baseline_rows` snapshot set-based from the F011 schedule read model, and one `baseline_row_measures` row per row and measure — all in one `UnitOfWork` — records `row_count`, emits `baseline.captured.v1`, and returns `201 BaselineResponse`; the 21st baseline returns `409 conflict`; `GET /api/v1/sheets/{sheet_id}/baselines` pages by `captured_at` through the repository `list` and returns measures from `baseline_measures`; `GET /api/v1/baselines/{id}/variance` is `page_variance(baseline_id, cursor, limit)`, which joins `baseline_rows` and `baseline_row_measures` to current rows and cells, computes `start_variance_days` and `finish_variance_days` with `working_days_between` on the sheet calendar, measure deltas, per-row `status` (`on_track`, `slipped`, `early`, `added`, `removed`), and `totals` aggregated over `baseline_row_measures`; the domain services hold no SQL and depend on the repository trait; errors map per ticket section 4; metric `baseline_capture_rows`.
