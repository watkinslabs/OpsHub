...
- Output/behavior: routes `PUT /api/v1/workspaces/{id}/members`, `GET /api/v1/workspaces/{id}/tree`, `POST /api/v1/folders`, `PATCH /api/v1/folders/{id}`, `POST /api/v1/folders/{id}/move`, `DELETE /api/v1/folders/{id}` return `MemberResponse[]`, `FolderTreeResponse { tree_version, roots }`, and `FolderResponse { id, workspace_id, parent_folder_id, name, path, depth, position, version, deleted_at }`; `path.rs` builds and rewrites materialized paths and rejects cycles before the trigger fires; `service_members.rs` locks the workspace row, enforces at least one `owner`, and rejects duplicate subjects; `tree.rs` fetches denies in one query and prunes subtrees during one walk; events `workspace-member.updated.v1`, `folder.updated.v1`, `folder.moved.v1`; errors map per ticket section 4.
- Dependencies: T017 schema, workspace service, and router; F003 ACL lookup `authz::denies_for(actor, ResourceKind::Folder, workspace)`.
- Feature flag: `F005_FEATURE`

## TDD

- Failing test first: `testing/features/F005/api/member_tests.rs::members_replace_requires_owner`, `::members_replace_rejects_duplicate_subject`, `::members_replace_emits_event`; `folder_tests.rs::folder_move_into_descendant_rejected`, `::folder_move_rewrites_descendant_paths`, `::folder_depth_eleven_invalid`, `::folder_delete_cascades_descendants`; `tree_tests.rs::tree_hides_denied_subtree`, `::tree_etag_matches_tree_version`; `permission_tests.rs::folder_viewer_mutation_denied`, `::folder_cross_tenant_not_found`, `::non_member_tree_not_found`
