...
- Owner: platform
- Branch: `t216-failure-tests`
- Decision references: `docs/architecture-decisions.md` sections 2, 2.1, 4, 7, 9; `docs/capability-contracts.md` row F054
...
- Owned paths: `testing/features/F054/api/failure_tests.rs`, `testing/features/F054/database/constraint_tests.rs`, `testing/features/F054/e2e/bridge.spec.ts`, `testing/features/F054/accessibility/bridge.a11y.spec.ts`, `testing/features/F054/performance/{run_bench.rs, run_list_bench.rs}`, `testing/features/F054/requirements/cases.md` (final traceability), `testing/features/F054/README.md`
...
- Data access: no test opens a connection or issues SQL; every fixture write and every assertion about stored state goes through the `crates/persistence/src/bridge/` repositories — seeded flows through `BridgeFlowRepository::replace_draft_steps`, the 100,000-run generator through `BridgeRunRepository::insert_run` and `BridgeRunStepRepository::append_step_attempt`, and step assertions through `list_steps_for_run`. `constraint_tests.rs` drives the database directly only through the migration under test to prove the child-table constraints: a second `trigger` row per flow, a duplicate `(flow_id, step_order)`, a config row whose kind does not match its step, a duplicate `bridge_flow_step_mappings` target field, a duplicate `bridge_flow_step_branches(branch_order)`, a `next_step_id` or branch target that names no step, a `bridge_runs` row whose `(flow_id, flow_version)` has no published version, cascade of step and child rows on flow delete with runs restricted, and that `bridge_flow_versions.steps_snapshot`, `bridge_runs.input`, and the two `bridge_run_steps` snapshots are the only `jsonb` columns in the module (decision section 2.1).
...
- Failing test first: `testing/features/F054/api/failure_tests.rs::step_timeout_marks_failed`, `::quota_exhaustion_dead_letters_run`, `::redelivered_message_creates_no_duplicate_steps`, `::waiting_run_resumes_after_worker_restart`, `::cancel_during_wait_marks_cancelled`; `testing/features/F054/database/constraint_tests.rs::second_trigger_step_row_rejected`, `::duplicate_step_order_rejected`, `::step_config_row_must_match_kind`, `::duplicate_mapping_target_field_rejected`, `::duplicate_branch_order_rejected`, `::branch_target_foreign_key_enforced`, `::run_requires_published_version_pin`, `::flow_delete_cascades_step_children_and_restricts_runs`; `testing/features/F054/e2e/bridge.spec.ts::build_publish_run_retry_flow`, `::viewer_is_read_only`; `testing/features/F054/accessibility/bridge.a11y.spec.ts::builder_and_console_have_no_serious_axe_violations`; `testing/features/F054/performance/run_list_bench.rs::run_list_100k_p95`
...
- [ ] Failure, database constraint, E2E, accessibility, and performance lanes pass; evidence stored under `testing/evidence/F054/`
