...
- Owned paths: `crates/persistence/src/runtime/health_probe.rs` (`PgHealthProbe` and the retention queries), `services/api/src/runtime/health.rs`, `services/realtime/src/runtime/health.rs`, `services/worker/src/runtime/{health.rs, retention.rs}`, `infra/backup/{backup.sh, restore.sh, restore.md, manifest.json}`, `infra/compose/docker-compose.yml` (backup sidecar), `Makefile` (`restore-drill`, `backup-now`), `testing/features/F004/{requirements/cases.md, e2e/restore_drill.spec.rs}`
- Contract/input: `GET /healthz` → `200 {"status":"ok"}`; `GET /readyz` → `ReadinessReport { status, components: { database, nats, object_storage, outbox } }` where each component is `{ status: "ok"|"error", latency_ms, reason? }`, checking the database with `PgHealthProbe::ping` within 500 ms, NATS `connected`, `HEAD` bucket `opshub-files`, and outbox lag < 60 s from `OutboxRepository::lag_seconds`; `backup.sh` runs `pg_basebackup -Ft -z` nightly and `archive_command` ships WAL to `s3://opshub-backups/wal/`; `restore.sh --target-time <ts>` restores base plus WAL into a scratch database; `manifest.json` records row counts for `tenants`, `users`, `outbox_events` at backup time; `runtime.retention` job deletes published outbox rows > 7 d and `job_runs` > 30 d through named `OutboxRepository` and `JobRunRepository` retention queries; the health handlers, the sweeper, and their tests contain no SQL string or `sqlx::query*` call, which stay inside `crates/persistence/src/runtime/` (decision 2.1); the backup and restore shell scripts are operator tooling outside the Rust crates and keep their `psql` counts.
