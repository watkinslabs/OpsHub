...
parent_story: S029
depends_on: [S029]
owned_paths: [crates/domain/src/templates/**, services/api/src/templates/**, services/api/migrations/*_templates_*.sql, testing/features/F015/api/**, testing/features/F015/database/**, testing/features/F015/requirements/**]
...
- Output/behavior: routes `GET /api/v1/project-templates`, `POST /api/v1/project-templates`, `GET /api/v1/project-templates/{id}`, `POST /api/v1/project-templates/{id}/versions` return `TemplateResponse { id, name, category, description, tags, is_builtin, current_version, versions: [VersionResponse], version }`; `validate_manifest` enforces key uniqueness, dependency references, F007 column types, and the FR-F015-03 limits with `field_errors.manifest.<path>`; publish sets `published_at`, updates `current_version_id`, and emits `template.published.v1`; DDL per ticket section 4 including the `template_versions_immutable` trigger, unique indexes, `manifest_bytes` check, and the seed of ten built-ins under the reserved tenant; `sqlx migrate revert` drops tables and trigger.
