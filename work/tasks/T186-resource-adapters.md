...
parent_story: S093
depends_on: [S093]
owned_paths: [crates/domain/src/mcp/**, crates/persistence/src/mcp/**, testing/features/F047/api/**, testing/features/F047/performance/**]
...
- Owner: platform
- Branch: `t186-resource-adapters`
- Decision references: `docs/architecture-decisions.md` sections 2, 2.1, 3, 8; `docs/capability-contracts.md` row F047
...
- Owned paths: `crates/domain/src/mcp/{uri.rs, descriptor.rs, redaction.rs, read_tools.rs}`, `crates/domain/src/mcp/adapters/{mod.rs, workspace.rs, document.rs, folder.rs, project.rs, task.rs, ticket.rs, dashboard.rs, workflow.rs, audit.rs}`, and `crates/persistence/src/mcp/resource_kind_repository.rs`.
...
- [ ] All nine adapters registered and reachable through `resources/list` and `resources/read`; every read goes through the owning feature's repository and no adapter contains SQL or opens a connection
