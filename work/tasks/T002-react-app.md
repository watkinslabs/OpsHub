...
parent_story: S001
depends_on: [T001]
owned_paths: [package.json, pnpm-workspace.yaml, apps/web/package.json, apps/web/vite.config.ts, apps/web/tsconfig.json, apps/web/src/main.tsx, apps/web/src/features/platform/**, testing/features/F001/frontend/**, testing/features/F001/accessibility/**]
...
Create the pnpm workspace and the Vite React 19 TypeScript application with TanStack Router and Query providers and a `/status` page that reads `GET /healthz`. Design tokens and the shared UI library are F062 and are not created here.

## Specification

- Owned paths: `package.json`, `pnpm-workspace.yaml`, `apps/web/package.json`, `apps/web/vite.config.ts`, `apps/web/tsconfig.json`, `apps/web/src/main.tsx`, `apps/web/src/features/platform/{routes.ts, StatusPage.tsx, useHealth.ts, api.ts}`
