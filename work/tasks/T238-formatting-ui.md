...
- Data access: the panel owns no persistence code and issues no SQL; it reads and writes only through the generated `FormattingApi`, whose `condition` AST, `target.column_ids` array, and `format` object (with `text_style` as an array) are unchanged by the normalized storage behind them, so the condition builder, target picker, and format picker keep their current payloads (decision section 2.1).
