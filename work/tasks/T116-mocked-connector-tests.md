...
- Owner: platform
- Branch: `t116-mocked-connector-tests`
- Decision references: `docs/architecture-decisions.md` sections 2, 2.1, 7, 9; `docs/capability-contracts.md` row F029
...
- Output/behavior: contract tests replay recorded responses through the adapters and assert parsed structures and error classes; negatives prove member denial, owner-only `notify-test`, `not_found` for foreign IDs, cross-tenant state rejection, and absence of token material in responses, logs, and audit rows; database tests prove one token row per connection, one active binding per sheet, cascade delete, `refresh_failures` bounds, and the normalized shape — `integration_connection_scopes` rejects a second row for the same `(connection_id, scope)` so granted and missing stay disjoint, `integration_connection_capabilities` rejects a capability absent from `integration_provider_capabilities` for that provider, `oauth_token_scopes` disappears with its token row, `integration_connection_errors` allows at most one row per connection, `integration_conflicts` rejects a duplicate `(event_id, field)` and cascades with its event, `calendar_event_links` rejects a duplicate `external_event_id` per binding and only accepts `review_state` in `clean`/`needs_review`, and `integration_events.detail` is the only remaining `jsonb` column in the module; the E2E flow connects Slack through the mock consent page, sends a test to `#ops`, connects Google, binds `Launch plan` with `newest_wins`, edits a date on both sides, and sees the conflict entry; accessibility runs axe on integrations routes and dialogs and checks the popup hand-off announcement; performance proves a 1,000-row calendar sync under 5 minutes with mocked `429`s and notification p95 under 3 s.
- Data access: no test file opens a connection or issues SQL of its own; every fixture write and every assertion about stored state goes through the `crates/persistence/src/integrations/` repositories, except the constraint tests, which deliberately drive the database directly through the migration harness to prove the constraints reject bad rows (decision section 2.1).
