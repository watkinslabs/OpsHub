...
- Data access: no test opens a connection or issues SQL of its own. `testing/fixtures/formatting.rs` seeds sheets, rules, condition trees, target columns, text styles, and cached matches through the `crates/persistence/src/formatting/` repositories and the F006 repositories, and every assertion about stored state reads back through `FormattingRuleRepository` and `FormattingStateRepository`. The database lane is the one exception and asserts constraints directly against the schema: `formatting_rule_conditions` rejects a duplicate `(rule_id, parent_id, position)`, a leaf without `column_id` or `op`, a group carrying either, and a `column_id` absent from `columns`; `formatting_rule_target_columns` rejects a duplicate column, a duplicate ordinal, and a foreign column reference, and its rows cascade when the rule is deleted; `formatting_rule_text_styles` rejects a duplicate style and a style outside `bold|italic|strikethrough`; `formatting_rules` rejects a fill, text colour, or icon token outside its `check` enum; and `formatting_rule_conditions.operand` is the only `jsonb` column left in the module (decision sections 2 and 2.1).
- Dependencies: T237 routes, schema, child tables, and events; T239 evaluator, read hook, worker jobs, and web rendering; the F035 fixed-clock evaluator; the F013 view fixtures; the shared schema-per-worker harness in `testing/harness/`.
...
- Failing test first: `testing/features/F060/accessibility/formatting.a11y.spec.ts::color_is_never_the_only_signal`, `::token_pairs_meet_contrast_thresholds`, `::formatted_row_describes_applied_rules`; `testing/features/F060/performance/formatting_bench.rs::evaluate_hundred_rules_over_five_hundred_rows_under_25ms`, `::materialize_hundred_thousand_rows_under_90s`, `::view_row_page_p95_within_ten_percent`; `testing/features/F060/database/constraint_tests.rs::condition_node_position_unique_per_parent`, `::condition_leaf_requires_column_and_operator`, `::condition_column_must_exist`, `::target_column_duplicate_and_ordinal_conflicts_rejected`, `::text_style_outside_enum_rejected`, `::format_token_outside_enum_rejected`, `::rule_delete_cascades_condition_target_and_style_rows`, `::operand_is_the_only_jsonb_column_in_the_module`; `testing/features/F060/requirements/traceability_tests.rs::every_requirement_id_has_a_named_test`, `::every_lane_cites_requirement_ids`
