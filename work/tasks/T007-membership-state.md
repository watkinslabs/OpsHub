...
- Output/behavior: routes `GET/POST /api/v1/users`, `PATCH /api/v1/users/{id}`, `POST /api/v1/users/{id}/deactivate`, `GET/POST /api/v1/groups`, `PATCH /api/v1/groups/{id}`, `PUT /api/v1/groups/{id}/members` return `UserResponse { id, email, display_name, status, external_id, last_login_at, group_ids, version, created_at, updated_at }` and `GroupResponse { id, name, description, member_count, version, ... }`; `status.rs` implements `UserStatus::transition`; deactivate calls `UserRepository::lock_tenant_for_admin_count` (the `SELECT ... FOR UPDATE` on the tenant row lives in the repository), counts active tenant-admins, calls `GroupRepository::remove_user_from_all_groups`, calls `SessionRevoker`, and emits `user.deactivated.v1`; membership replace computes `added_user_ids` and `removed_user_ids` in the domain and calls `GroupRepository::replace_members`, which deletes and bulk-inserts inside the one `UnitOfWork` transaction, and emits `group.updated.v1`; `TenantSettingsForm.tsx` edits the four typed `tenant_settings` fields through the generated client; the domain services, handlers, React pages, and tests contain no SQL (decision 2.1); the React pages implement the states, dialogs, and telemetry from ticket section 3.
