...
parent_story: S067
depends_on: [S067]
owned_paths: [crates/domain/src/workload/**, services/api/src/workload/**, services/api/migrations/*_workload_*.sql, testing/features/F034/database/**]
...
- Owner: platform
- Branch: `t134-time-entry-tracking`
- Decision references: `docs/architecture-decisions.md` sections 2, 4, 7; `docs/capability-contracts.md` row F034
...
- Owned files: `services/api/migrations/<ts>_workload_create_tables.sql` and `.down.sql`, `crates/domain/src/workload/{time_entry.rs, effort.rs, service.rs, errors.rs}`, `services/api/src/workload/{handlers_time_entries.rs, handlers_import.rs, handlers_reconcile.rs, dto.rs}`.
- DDL: `time_entries`, `effort_summaries`, and `workload_conflicts` exactly as in ticket section 4, including `check (hours between 0.25 and 24 and (hours * 4) = floor(hours * 4))`, `check (source in ('native','external'))`, `check (reconciliation_state in ('none','pending','accepted','rejected','superseded'))`, `check ((source = 'external') = (source_system is not null and external_id is not null))`, the partial unique `time_entries_external_ref_idx on (tenant_id, source_system, external_id) where source = 'external' and deleted_at is null`, unique `(resource_id, period_start)` on `workload_conflicts`, restrict foreign keys to `resources`, `rows`, and `sheets`, and a statement trigger rejecting an `UPDATE` of `reconciled_by`, `reconciled_at`, `resolution`, or `reason` once set.
...
- Rollback: disable `F034_FEATURE`, then run the down migration on an empty tenant, which drops the three tables and the trigger.
