...
- Output/behavior: `POST /api/v1/sheets/{sheet_id}/cells/bulk` and `POST /api/v1/sheets/{sheet_id}/rows/bulk` resolve the selection (`resolve_selection` caps at 1,000 rows / 5,000 cells, over-cap returns `400 invalid` with `field_errors.selection = "too_large"`), lock rows, normalize per F007, write cells and `cell_history`, one `edit_batches` row, aggregate audit row, and one `cells.bulk-updated.v1` or `rows.bulk-updated.v1` event in one transaction, returning `BulkResponse { batch_id, applied, invalid, conflict, row_versions }`; `fill.rs` continues integer, decimal, and date sequences from `source_cell` and repeats text; `GET /api/v1/sheets/{sheet_id}/changes` reads `cells` where `change_version > since` ordered ascending and returns `ChangesResponse { changes, next_since, layout }`; `GET /api/v1/cells/{row_id}/{column_id}/history` pages `cell_history` newest first; `save_user_layout` upserts `sheet_user_layouts`, rejects hidden primary column with `field_errors.layout.hidden_columns`, bounds `frozen_column_count` 0–5.
- Dependencies: T029 schema, batch, and inverse code; F007 normalizer; F006 row lock helper; F003 authz; F004 outbox.
- Feature flag: `F008_FEATURE`

## TDD

- Failing test first: `testing/features/F008/api/bulk_tests.rs::bulk_cells_applies_and_emits_one_event`, `::bulk_cells_rejects_over_5000`, `::bulk_cells_fill_continues_date_sequence`, `::bulk_rows_returns_row_versions`, `::bulk_batch_undo_reverts_all_cells`; `testing/features/F008/api/feed_tests.rs::changes_feed_orders_by_change_version`, `::changes_feed_returns_actor_layout`; `testing/features/F008/api/history_tests.rs::history_pages_newest_first`, `::history_cross_tenant_not_found`; `testing/features/F008/api/layout_tests.rs::layout_upsert_rejects_hidden_primary`, `::layout_is_private_per_user`
