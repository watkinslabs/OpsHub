...
parent_story: S067
depends_on: [S067]
owned_paths: [crates/domain/src/workload/**, crates/persistence/src/workload/**, services/api/src/workload/**, services/worker/src/workload/**, testing/features/F034/api/**]
...
- Owner: platform
- Branch: `t133-workload-query`
- Decision references: `docs/architecture-decisions.md` sections 2, 2.1, 3, 9; `docs/capability-contracts.md` row F034
...
- Owned files: `crates/domain/src/workload/{mod.rs, workload_row.rs, conflict.rs, suggestion.rs, errors.rs}`, `services/api/src/workload/{mod.rs, routes.rs, handlers_workload.rs, handlers_conflicts.rs, dto.rs}`, `services/worker/src/workload/{mod.rs, conflict_detector.rs}`, `crates/persistence/src/workload/{mod.rs, conflict_repository.rs}` (`effort_summary_repository.rs` is T134's file and is consumed, not edited, here).
...
- Conflict detector: for each `(resource_id, period)` in the event span, compare `allocated_hours` from F033 allocations against `available_hours`; where allocated exceeds available, call `WorkloadConflictRepository::upsert_open_conflict` on `(resource_id, period_start)` with `over_hours = allocated − available`, `status: open`, `detected_at`, and `replace_conflict_allocations` with the contributing allocations, which inserts the missing `workload_conflict_allocations` rows and deletes the departed ones in the same `UnitOfWork` (the conflicts DTO still returns them as an ascending `allocation_ids` array); publish `workload-conflict.detected.v1` only on the `open` transition. Periods no longer over move to `status: resolved` with `resolved_at` and write audit `workload-conflict.resolve`. Idempotent by `(scope_id, source_version)`; 3 retries then dead letter with `last_error`; spans carry `tenant_id`, `resource_id`, `correlation_id`; metric `conflict_detection_ms` exported.
- Data access (decision section 2.1): `handlers_workload.rs`, `handlers_conflicts.rs`, `conflict.rs`, `suggestion.rs`, and `conflict_detector.rs` contain no SQL string, `sqlx::query*` call, or connection. `WorkloadConflictRepository` owns `workload_conflicts` and `workload_conflict_allocations`; `EffortSummaryRepository` owns `effort_summaries` and `effort_summary_sources`; the workload read uses `query_workload_page`, the conflicts list uses `list_conflicts_page` plus `list_conflict_allocations`, the detector uses `upsert_open_conflict`, `replace_conflict_allocations`, and `resolve_periods_no_longer_over`, and float and reassign candidates come from the F012 `ScheduleResultRepository` and the F033 `AllocationRepository`/`ResourceRepository`.
...
- Failing test first: `testing/features/F034/api/workload_tests.rs::workload_rows_carry_utilization_and_status`, `::utilization_is_null_when_available_is_zero`, `::workload_range_over_182_days_is_invalid`, `::workload_over_500_resources_is_invalid`, `::workload_row_reports_stale_when_source_event_queued`, `::non_viewer_sees_only_own_workload_row`; `testing/features/F034/api/conflict_tests.rs::capacity_event_opens_conflict_with_over_hours`, `::conflict_detection_is_idempotent_per_source_version`, `::resolved_period_sets_resolved_at_and_stops_publishing`, `::conflict_lists_shift_and_reassign_suggestions`, `::conflict_allocation_rows_replace_on_recompute`, `::reassign_candidates_capped_at_three_by_remaining_hours`, `::foreign_tenant_conflict_is_not_found`
