...
parent_story: S042
depends_on: [S042]
owned_paths: [crates/domain/src/reports/**, services/api/src/reports/**, apps/web/src/features/reports/**, testing/features/F021/api/**, testing/features/F021/frontend/**]
...
- Output/behavior: routes `GET /api/v1/reports`, `POST /api/v1/reports`, `GET /api/v1/reports/{id}`, `PATCH /api/v1/reports/{id}`, `DELETE /api/v1/reports/{id}`, `GET /api/v1/reports/{id}/rows`, `POST /api/v1/reports/{id}/refresh` return `ReportResponse { id, workspace_id, folder_id, name, description, definition, refresh_policy, aggregate_policy, latest_snapshot, version, created_at, updated_at, deleted_at }`, `ReportRowsResponse { rows, meta, next_cursor }`, `RefreshResponse { run_id, status }`; `filters.rs` resolves relative date tokens in the policy timezone; `grouping.rs` groups visible rows at read time, computes aggregates excluding hidden values unless `aggregate_scope = owner`, and emits header rows; errors map per ticket section 4; events `report.created.v1`, `report.updated.v1`, `report.deleted.v1` written to the outbox in the same transaction; the React pages render every state from ticket section 3 and emit telemetry `report_created`, `report_opened`, `report_refresh_requested`, `report_definition_saved`, `report_restricted_sources_shown`.
