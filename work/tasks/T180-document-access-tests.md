...
parent_story: S090
depends_on: [T179]
owned_paths: [crates/domain/src/documents/**, crates/persistence/src/documents/**, services/api/src/documents/**, testing/features/F045/api/**, testing/features/F045/e2e/**, testing/features/F045/requirements/**]
...
- Owned paths: `crates/domain/src/documents/{access.rs, principal.rs}`, `crates/persistence/src/documents/{document_repository.rs, document_search_repository.rs}`, `services/api/src/documents/{authz.rs, rate_limit.rs}`, `testing/features/F045/api/access_tests.rs`, `testing/features/F045/e2e/documents.spec.ts`, `testing/features/F045/requirements/cases.md`
...
- Output/behavior: `resolve_effective_access` reads the node's ancestor chain from `document_ancestors` through `DocumentRepository`, loads grants for every ancestor ID plus the node in one query, returns the highest role walking root-to-leaf (`distance` descending), and returns `denied` when any level has `deny = true`; denied and missing access map to `404 not_found`; `PrincipalKind::{Guest, Link}` receive `403 denied` on root listings and on every mutation route; link principals pass through the F038 limiter keyed `link:{token_id}` at 60 requests per minute returning `429 rate_limited` with `Retry-After`; expired or revoked links return `404 not_found`; search results drop `search_visibility = hidden` nodes and link-only nodes unless `link_search_discoverable` is true; the `EffectiveAccessCache` memoizes walks per request. The access tests drive the repository traits rather than raw queries; no SQL string, `sqlx::query*` call, or connection exists in `crates/domain/src/documents`, `services/api/src/documents`, or the test files.
