...
- Fixtures/mocks: Playwright uses the real API against seeded tenants A and B; the `ja-JP` fixture catalog deliberately omits one key's entry row to exercise the per-key fallback; axe-core via Playwright
