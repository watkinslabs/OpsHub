...
- **SR-S122-01:** `GET /public/update-requests/{token}` runs with no session, resolves the recipient by constant-time comparison of the SHA-256 `token_hash` through `UpdateRequestRecipientRepository::find_active_by_token_hash`, marks the recipient `opened` with `opened_at` on first load through `mark_opened`, reads the scope with `UpdateRequestRepository::load_scope` over `update_request_scope_rows` and `update_request_scope_fields`, and returns only `row_key`/`field_key` addressed cells with labels, types, options, validation, current values, and `row_version` (covers FR-F061-04, FR-F061-03).
...
- **SR-S122-03:** `POST /public/update-requests/{token}/responses` requires `Idempotency-Key`, validates every value with the F007 column validator, returns `field_errors.<row_key>.<field_key>` on failure, returns `404 not_found` for any key outside the scope snapshot — a check the composite foreign keys from `update_request_response_values` and `update_request_response_row_versions` to the two scope tables also enforce in the database — stores the submitted `values` and `row_versions` maps as rows through `UpdateRequestResponseRepository::insert_received_with_values` while keeping the request body shape unchanged, and is limited to 30 submissions per hour per token and IP and 300 per day per token with `429 rate_limited` and `Retry-After` (FR-F061-05).
...
- **SR-S122-05:** `submit: false` stores a resumable draft for 7 days — a `draft` response plus its `update_request_response_values` rows, replaced wholesale by `UpdateRequestResponseRepository::save_draft` — that writes no cells; `submit: true` over part of the scope is accepted when `allow_partial` and marks the recipient `partial`; with `allow_partial` false a gap returns `400 invalid` with reason `incomplete` and writes nothing (FR-F061-07).
- **SR-S122-06:** A submitted `row_versions.<row_key>` that no longer matches the row returns `409 conflict` listing each stale row with its current version and values, records the response as `rejected` with reason `stale_row` and keeps its `update_request_response_row_versions` rows so the rejection is reconstructable per row, and writes no cell of that submission (FR-F061-08).
...
- Data access: `crates/persistence/src/update-requests/{request_repository.rs, recipient_repository.rs, response_repository.rs, reminder_repository.rs}` hold every SQL statement behind the two unauthenticated routes — token lookup, `opened_at`, scope read, response and value inserts, idempotency replay, and the reminder cancellation on first response; `public.rs`, `response.rs`, `validate.rs`, and `handlers_public.rs` depend on the repository traits and contain no `sqlx::query*` call, and one submission's response row, value rows, row-version rows, recipient status, and F008 cell writes commit in a single `UnitOfWork` (decision section 2.1)
...
- Data/migration: reads `update_request_scope_rows`, `update_request_scope_fields`, and `update_request_recipients.token_hash`; writes `update_request_responses`, `update_request_response_values`, and `update_request_response_row_versions` under the append-only trigger from ticket section 4 (migration itself is owned by T241)
...
- First failing tests: `public_scope_omits_internal_identifiers`, `revoked_and_expired_tokens_return_same_not_found`, `field_key_outside_scope_returns_not_found`, `response_values_stored_one_row_per_cell`, `draft_writes_no_cells_and_resumes`, `partial_submit_marks_recipient_partial`, `incomplete_submit_rejected_when_partial_disabled`, `stale_row_version_returns_conflict_with_current_values`, `submission_replay_returns_original_result`, `public_form_completable_by_keyboard`
