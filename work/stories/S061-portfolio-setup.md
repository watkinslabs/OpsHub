...
owned_paths: [crates/domain/src/portfolios/**, crates/persistence/src/portfolios/**, services/api/src/portfolios/**, services/api/migrations/*_portfolios_*.sql, testing/features/F031/**]
...
- Owner: platform
- Branch: `s061-portfolio-setup`
- Decision references: `docs/architecture-decisions.md` sections 2, 2.1, 3, 4; `docs/capability-contracts.md` row F031
...
- **SR-S061-03:** `PATCH /api/v1/portfolios/{id}` requires `If-Match`, accepts `name`, `description`, `refresh_policy`, `stale_after_seconds`, and `measure_mappings`, which `PortfolioRepository::replace_measure_mappings` stores as one `portfolio_measure_mappings` row per measure and returns as the same JSON object; a stale version returns `409 conflict` with `current_version`, and a mapping with an unknown measure key returns `400 invalid` with `field_errors.measure_mappings` (FR-F031-03, FR-F031-05).
...
- Data access: `crates/persistence/src/portfolios/{mod.rs, portfolio_repository.rs, rollup_repository.rs}` hold every SQL statement for this slice; the domain service and the `services/api/src/portfolios` handlers depend on the repository traits and contain no `sqlx::query*` call (decision section 2.1)
...
- Data/migration: `services/api/migrations/<ts>_portfolios_create_tables.sql` creating `portfolios`, `portfolio_projects`, `portfolio_measure_mappings`, `portfolio_rollups`, `portfolio_rollup_rows`, and `portfolio_rollup_totals` with the constraints and indexes from ticket section 4
