...
parent_feature: F018
depends_on: [S035]
owned_paths: [crates/domain/src/workflows/**, crates/persistence/src/workflows/**, services/api/src/workflows/**, apps/web/src/features/workflows/**, testing/features/F018/**]
...
- **SR-S036-02:** `resolve_placeholders(template, row, event, actor)` substitutes `{{row.<column_id>}}`, `{{event.<field>}}`, `{{actor.<field>}}` using display values over an already-loaded row and holds no SQL; unknown column IDs fail validation with the placeholder text in `field_errors.actions[i].params`, and every resolved `{{row.<column_id>}}` is collected for the publish-time `workflow_step_column_refs` projection with `usage = 'action'` (FR-F018-05).
- **SR-S036-03:** `WorkflowVersionRepository::publish(workflow_id, definition)` freezes the draft, inserts the `workflow_versions` row, its `workflow_steps`, and its `workflow_step_column_refs` (trigger and condition refs with a null `step_id`, action refs carrying their step), and repoints `workflows.published_version_id` in one `UnitOfWork`, emitting `workflow.published.v1`; a later `PATCH` edits only `workflows.draft` (FR-F018-06, FR-F018-07).
- **SR-S036-04:** `POST /api/v1/workflows/{id}/disable` sets `state: disabled` and emits `workflow.disabled.v1`; `DELETE` soft deletes through `WorkflowRepository::soft_delete` and leaves published versions readable by ID through `load_version(version_id)`; `list_workflows_using_column(column_id)` reports the workflows blocking an F007 column delete (FR-F018-08, FR-F018-11).
- **SR-S036-05:** Publishing the 101st workflow on a sheet or the 5,001st in a tenant returns `409 conflict` with `field_errors.limit`, counted inside the publish `UnitOfWork` (FR-F018-12).
...
- Rust service/API: `crates/domain/src/workflows/{action.rs, placeholders.rs, publish.rs, service_versions.rs}` (repository traits only, no SQL); `crates/persistence/src/workflows/{mod.rs, workflow_version_repository.rs}`; `services/api/src/workflows/{handlers_publish.rs}`
- Data/migration: none new; uses `workflows`, `workflow_versions`, `workflow_steps`, `workflow_step_column_refs` from S035
