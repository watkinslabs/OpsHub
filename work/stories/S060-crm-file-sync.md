...
parent_feature: F030
depends_on: [F030]
owned_paths: [crates/domain/src/connectors/**, services/worker/src/connectors/**, services/api/src/connectors/**, apps/web/src/features/connectors/**, testing/features/F030/**]
...
- **SR-S060-03:** The Tableau adapter publishes a F023 report result as a Hyper extract to a named project and datasource, replacing the datasource per run and storing the returned LUID; the `database` adapter reads only, using a secret-manager DSN, a `schema.table` or named-query allowlist, parameterized statements, a 30 s statement timeout, and a 50,000-row cap that ends the run `partial` with code `row_cap_reached`; any DDL or DML attempt is rejected before execution (FR-F030-19, NFR-F030-02).
- **SR-S060-04:** `sync_cursors` holds one row per `(sync_id, direction)`, the engine checkpoints the cursor and record links every 500 records inside the same transaction as the applied rows, a restarted worker resumes from `checkpoint_record_id` without reprocessing the page, and `reset_cursor_to` on `PATCH /api/v1/syncs/{id}` writes the `sync.cursor-reset` audit event (FR-F030-09, NFR-F030-04).
- **SR-S060-05:** Errors classify as `transient`, `permanent`, or `mapping`; `transient` retries five times at 1 s, 2 s, 4 s, 8 s, 16 s with ±20% jitter honoring `Retry-After`, then dead-letters; a run finalizes `completed` at zero failures, `partial` under 10%, and `failed` at 10% or more, which sets the sync `state: error`, pauses its schedule, and publishes `sync-run.failed.v1` (FR-F030-10, NFR-F030-04).
- **SR-S060-06:** `POST /api/v1/sync-runs/{id}/replay` restarts from the source run's `cursor_before`, or over just its failed records with `only_failed`, and with `dry_run` returns `{ would_create, would_update, would_skip, would_conflict, samples }` without writing; idempotency comes from `sync_record_links` compared on `external_version` so an already-applied record counts as `skipped` (FR-F030-12).
- **SR-S060-07:** A record whose OpsHub row and external counterpart both changed since the cursor produces a `sync_conflicts` row with per-field `opshub_value` and `external_value`, both timestamps, and `state: open`, publishing `sync-conflict.detected.v1`; `manual` leaves both sides untouched while `opshub_wins`, `external_wins`, and `newest_wins` apply the winner and store `state: auto_resolved` with the resolution (FR-F030-13).
- **SR-S060-08:** `GET /api/v1/syncs/{id}/conflicts` filters by `state`, and `POST /api/v1/sync-conflicts/{id}/resolve` applies `keep_opshub`, `keep_external`, or `merge` with `expected_version`, requires `field_values` covering every conflicting field for `merge`, publishes `sync-conflict.resolved.v1`, and returns `409 conflict` for an already-settled conflict (FR-F030-14).
- **SR-S060-09:** Run logs store field values as SHA-256 digests unless `debug_payloads` is set, which retains raw samples for 7 days and writes the `sync.debug-payloads-enabled` audit event; metrics `sync_records_total`, `sync_run_duration_seconds`, `sync_conflicts_open`, and `sync_cursor_lag_seconds` are emitted and every run carries a tracing span with `sync_id` and `run_id` (NFR-F030-02, NFR-F030-04).
...
- Data/migration: no new tables; this story fills `sync_cursors`, `sync_conflicts`, and `sync_record_links` created by the S059 migration and adds the nightly retention sweep
