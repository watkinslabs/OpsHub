...
- **SR-S124-10:** One visual system: every rendered element is a themed MUI component, with MIT MUI X Charts and Date Pickers for charts and dates; TanStack Table and Virtual supply headless state and virtualization only and emit no markup or styles of their own; no other UI library is in the tree and no licensed grid package or licence key exists anywhere (FR-F062-16).
- **SR-S124-11:** `DataGridPanel` supplies virtualization at F008's 100,000-row budget, resize, reorder, hide and freeze from TanStack `columnSizing`, `columnOrder`, `columnVisibility` and `columnPinning` state persisted through F008's `layout` field, range and non-contiguous selection with TSV clipboard copy, and `role="grid"` with correct aria indices on virtualized cells; grouping, tree rows, aggregation and export stay server-owned (FR-F062-17, FR-F008-10, FR-F008-13, FR-F008-14).
