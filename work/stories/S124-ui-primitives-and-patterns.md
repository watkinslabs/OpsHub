...
- **SR-S124-10:** The grid is MUI X Pro for every tenant with no entitlement consulted and no upgrade prompt; the licence key is injected at build time from `mui/x-license-key`, never fetched at runtime, and a production build without it fails naming the secret while development and CI run watermarked on the evaluation (FR-F062-16).
- **SR-S124-11:** `DataGridPanel` uses the Pro pinning, reorder, range-selection and clipboard capabilities and binds them to OpsHub state, persisting order, widths, hidden set and frozen count through F008's `layout` field with a 1-second debounce, while grouping, tree rows, aggregation and export stay server-owned; each behaviour has a keyboard path and an accessible announcement (FR-F062-17, FR-F008-10, FR-F008-14).
