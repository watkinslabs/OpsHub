...
- **SR-S124-10:** The entitlement covers presentation only: grouping, tree rows, aggregation and xlsx export come from F013, F009, F022 and F010 and work at every tier, and the grid never re-implements them. `useProEntitlement` reads `data-grid-pro` through F048 and returns `{ allowed: false, reason: "not_entitled" }` until F048 ships; unentitled, only column reorder, column pinning, row pinning and range selection render as disabled affordances linking to `/admin/entitlements`, the paid chunk is never loaded, and no console error appears (FR-F062-16).
- **SR-S124-11:** The entitled path is implemented and exercised: reorder writes the view's `settings.columns`, pinning persists per viewer, range selection copies TSV, each has a keyboard equivalent and an announcement, and the story matrix runs in both entitlement states with the entitled lane skipping loudly when CI has no licence key (FR-F062-17).
...
- First failing tests: `direct_mui_import_fails_lint`, `paid_affordances_disabled_without_entitlement`, `paid_bundle_absent_by_default`, `column_reorder_persists_to_view_settings`, `data_grid_virtualizes_above_one_hundred_rows`, `chart_palette_is_fixed_and_labelled`, `error_state_renders_correlation_id_and_retry`, `app_shell_rail_width_persists_across_reload`, `icons_import_only_through_registry`, `formatted_date_uses_explicit_locale`, `ui_modules_perform_no_network_call`, `themed_bundle_under_budget`
