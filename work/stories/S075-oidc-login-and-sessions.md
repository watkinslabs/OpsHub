...
- **SR-S075-03:** A successful callback inserts `sessions` and `refresh_tokens` through `SessionRepository` in one `UnitOfWork`, sets `__Host-oh_session`, updates `last_login_at`, emits `session.created.v1`, and redirects only to same-origin `return_to` paths (FR-F038-03).
...
- **SR-S075-06:** TOTP enrol/verify and WebAuthn register/assert write `mfa_factors` through `MfaFactorRepository`, set `mfa_verified_at` through `SessionRepository`, enforce the 5-factor cap and the last-factor rule against `SecurityPolicyRepository`, and emit `mfa.enrolled.v1` / `mfa.removed.v1` (FR-F038-07, FR-F038-08, FR-F038-09).
...
- Rust service/API: `crates/auth/src/auth/{mod.rs, context.rs, extract.rs, audit_sink.rs, cipher.rs}`; `crates/domain/src/auth/{mod.rs, session.rs, refresh.rs, oidc.rs, provider.rs, factor.rs, totp.rs, webauthn.rs, errors.rs, service_session.rs, service_mfa.rs}`; `services/api/src/auth/{mod.rs, routes.rs, handlers_oidc.rs, handlers_session.rs, handlers_mfa.rs, dto.rs}`; every table is reached through `SessionRepository` (`sessions`, `refresh_tokens`), `MfaFactorRepository` (`mfa_factors`), `ApiTokenRepository` (`api_tokens`, `api_token_scopes`), `SecurityPolicyRepository` (`security_policies`, `security_policy_email_domains`), and `RateLimitBucketRepository` (`rate_limit_buckets`) in `crates/persistence/src/auth/`; `crates/auth`, `crates/domain/src/auth/`, `services/api/src/auth/`, and the tests contain no SQL and depend on those repository traits and the shared `UnitOfWork` (decision 2.1)
- Data/migration: `services/api/migrations/<ts>_auth_create_tables.sql` creating all eight tables from ticket section 4 — `sessions`, `refresh_tokens`, `mfa_factors`, `api_tokens`, `api_token_scopes`, `security_policies`, `security_policy_email_domains`, `rate_limit_buckets`, with scopes and allowed domains as child rows rather than array columns (tokens, scopes, policy, domains, and buckets are used by S076)
