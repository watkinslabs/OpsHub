...
- **SR-S071-02:** `PATCH /api/v1/shares/{id}` requires `If-Match` and publishes `share.updated.v1`; `DELETE` publishes `share.revoked.v1`; revoking or downgrading the last `owner` grant returns `409 conflict` with `field_errors.role = "last_owner"` (FR-F036-02, FR-F036-03).
- **SR-S071-03:** `ShareGrantSource` in `crates/auth/src/sharing/` evaluates deny-wins then closest-allow over the `target → folder → workspace` chain for the actor and its groups; no grant means deny; evaluation errors fail closed (FR-F036-04, NFR-F036-04).
...
- **SR-S071-05:** Expired grants are ignored by evaluation and swept hourly into `share.revoked.v1` with `reason = expired` (FR-F036-13).
...
- **SR-S071-07:** Every mutation checks `Idempotency-Key`, writes an audit row, and enqueues its outbox event; editors receive `403 denied` on every share route and foreign tenants `404 not_found`; evaluation adds ≤ 5 ms p95 (FR-F036-15, NFR-F036-01).

## Surfaces

- Infrastructure/container: hourly `sharing.sweep_expired` job registered in the API process scheduler
- Rust service/API: `crates/domain/src/sharing/{share.rs, role.rs, evaluate.rs, errors.rs, service.rs, sweeper.rs}`; `crates/auth/src/sharing/{grant_source.rs}`; `services/api/src/sharing/{routes.rs, handlers_share.rs, dto.rs}`
...
- [ ] Production call path named: `services/api/src/sharing/routes.rs` mounted in `services/api/src/router.rs`; `crates/auth/src/sharing/grant_source.rs` registered in `crates/auth/src/engine.rs`; `apps/web/src/features/sharing/ShareDialog.tsx` opened from the sheet header `Share` button
