...
- **SR-S025-01:** `POST /api/v1/views` with `{ sheet_id, name, kind, visibility, settings }` validates settings per kind, stores the view in `views`, and returns `ViewResponse` with version 1 and `owner_id`; the 101st view on a sheet returns `400 invalid` with `field_errors.sheet_id = "view_limit"` (covers FR-F013-01, FR-F013-04).
- **SR-S025-02:** `compile_filter` accepts `and`/`or` groups with up to 50 leaves and operators matched to column type, and rejects unknown columns, mismatched operators, or a 51st leaf with `field_errors.settings.filter` (FR-F013-02, FR-F013-03).
- **SR-S025-03:** `GET /api/v1/views/{id}/rows` returns cursor pages of at most 500 rows in group then sort order, includes visible columns plus the primary column, and excludes rows and cells the actor cannot read (FR-F013-05).
- **SR-S025-04:** A card view requires a `select` lane column; `CardView` renders one `CardLane` per option, and a keyboard or pointer lane move calls `PATCH /api/v1/sheets/{sheet_id}/cells` with `If-Match` and rolls back on `conflict` (FR-F013-04, FR-F013-07).
...
- **SR-S025-06:** `PATCH` and `DELETE` honour `If-Match`, owner or `sheet-editor` rules, clear the previous default when `is_default` is set, and refuse deleting the default view (FR-F013-08, FR-F013-09).
...
- Rust service/API: `crates/domain/src/views/{view.rs, settings.rs, filter.rs, errors.rs, service.rs, service_rows.rs}`; `services/api/src/views/{routes.rs, handlers_view.rs, handlers_rows.rs, dto.rs}`
- Data/migration: `services/api/migrations/<ts>_views_create_tables.sql` creating `views` and `view_shares` with the indexes and checks from ticket section 4
...
- First failing tests: `view_create_returns_version_one`, `view_filter_rejects_type_mismatch`, `view_rows_exclude_hidden_rows`, `card_view_requires_select_lane_column`, `card_lane_move_patches_cell`, `calendar_drag_calls_reschedule`, `view_default_delete_invalid`
