...
- **SR-S121-07:** Recipient and request completion are recomputed after every applied response: a recipient is `completed` when its whole scope is filled, the request is `completed` when every scoped pair has a value from any recipient, and both transitions cancel outstanding reminders (FR-F061-09).
...
- Data/migration: `services/api/migrations/<ts>_update-requests_create_tables.sql` creating `update_requests`, `update_request_recipients`, `update_request_responses`, and `reminder_schedules` with the constraints and indexes in ticket section 4
