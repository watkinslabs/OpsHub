...
- **SR-S003-03:** `GET /api/v1/tenants/{id}` and `PATCH /api/v1/tenants/{id}` require the caller's own tenant id; `PATCH` requires `If-Match` and returns `409 conflict` with `current_version` when stale (FR-F002-03).
...
- **SR-S003-07:** The `users`, `groups`, and `group_members` tables and their constraints exist from this story's migration so S004 adds no schema (FR-F002-05, FR-F002-09 prerequisites).

## Surfaces

- Infrastructure/container: none beyond the F001 CI PostgreSQL 18 service
- Rust service/API: `crates/domain/src/tenants/{mod.rs, tenant.rs, slug.rs, errors.rs, service_tenant.rs, hooks.rs}`; `services/api/src/tenants/{mod.rs, routes.rs, handlers_tenant.rs, dto.rs, gate.rs}`
- Data/migration: `services/api/migrations/<ts>_tenants_create_tables.sql` creating `tenants`, `users`, `groups`, `group_members` with indexes and the same-tenant trigger from ticket section 4
...
- First failing tests: `tenant_create_bootstraps_admin`, `tenant_slug_taken_conflicts`, `tenant_invalid_region_rejected`, `tenant_stale_version_conflicts`, `tenant_suspend_blocks_api_routes`, `tenant_cross_tenant_not_found`
