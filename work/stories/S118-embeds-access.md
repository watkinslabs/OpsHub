...
parent_feature: F059
depends_on: [S117]
owned_paths: [crates/domain/src/publishing/**, services/api/src/publishing/**, apps/web/src/features/publishing/**, testing/features/F059/**]
...
- Owner: platform
- Branch: `s118-embeds-access`
- Decision references: `docs/architecture-decisions.md` sections 3, 4, 6, 9; `docs/capability-contracts.md` row F059
...
- **SR-S118-01:** `GET /embed/{token}` renders the snapshot in an iframe-safe document with `Content-Security-Policy: frame-ancestors` built from `allowed_origins`, renders the `denied` state for unlisted origins (checking `Origin`, `Referer`, or the signed `origin` query), and returns `404` when embedding is disabled (FR-F059-07).
...
- **SR-S118-03:** Every render records a sampled `publication_views` row (one per token per minute) with salted client hash and referrer origin and publishes `publication.viewed.v1` at most once per token per 5 minutes (FR-F059-10).
...
- **SR-S118-05:** `PublishDialog` shows the token and iframe snippet once with copy actions, edits expiry, access, origins, refresh interval, and freshness display; `PublicationsListPage` shows status, `view_count_7d`, `last_viewed_at`, and `Rotate` and `Revoke` actions (FR-F059-13).
...
- Infrastructure/container: rate-limit buckets reuse the F038 `rate_limit_buckets` store keyed by token hash and client hash
...
- Data/migration: none new; uses tables from S117
...
- First failing tests: `embed_sets_frame_ancestors_from_origins`, `embed_unlisted_origin_denied_state`, `tenant_access_other_tenant_not_found`, `token_on_api_route_denied`, `render_rate_limited_after_60_per_minute`, `publish_dialog_reveals_token_once`, `render_dashboard_12_widgets_p95`
