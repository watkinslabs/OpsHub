...
- **SR-S074-01:** `GET /api/v1/notification-preferences` returns the effective `{ channels: { <category>: { in_app, email, push } }, digest: { cadence, send_at_local, timezone }, quiet_hours: { start_local, end_local, timezone, enabled }, version }` — the same nested JSON body on the wire, composed by `NotificationPreferenceRepository` from the `notification_channel_preferences` rows and the typed `digest_*` and `quiet_hours_*` columns — resolved by `resolve_effective_preferences(tenant_id, user_id)` from the user row, the tenant-default row, and built-in defaults, and `PUT` replaces it under `If-Match` through `replace_channel_preferences(preference_id, rows)` plus the parent update in one `UnitOfWork`, returning `409 conflict` on a stale `version` (covers FR-F037-08).
- **SR-S074-02:** `in_app` for `approval` and `system` cannot be turned off — a submitted row with `enabled = false` for that pair is rejected before `replace_channel_preferences` runs and returns `400 invalid` with `field_errors.channels`, and a `PUT` with `scope = tenant` writes tenant defaults only for a `tenant-admin`, with every other caller denied (FR-F037-08, FR-F037-14).
- **SR-S074-03:** When `quiet_hours_enabled` is true and creation falls inside `[quiet_hours_start, quiet_hours_end)` in `quiet_hours_timezone`, the one window that applies on every weekday, email and push deliveries are queued with `next_attempt_at` at the end of the window and the `quiet_hours_release` job re-queues them; in-app is never delayed and `system` ignores quiet hours, including across the Europe/Berlin and America/New_York DST transitions (FR-F037-09).
- **SR-S074-04:** With `digest_cadence` `hourly` or `daily`, email deliveries outside `approval` and `system` are stored `digested` and `send_digest` sends one email per recipient at `digest_schedules.next_run_at` listing up to 200 items grouped by category, marks them `sent`, publishes `digest.sent.v1 { recipient_id, item_count }`, and calls `DigestScheduleRepository::advance_digest(schedule_id, next_run_at)` in the same `UnitOfWork` as the delivery updates; an empty window sends nothing and still advances (FR-F037-10).
- **SR-S074-05:** `send_digest` takes schedules only from `DigestScheduleRepository::claim_due_digests(now, limit)`, whose `for update skip locked` claim on the schedule row still serializes the writers, so two workers never send the same digest and a re-run of the same window sends nothing further; the job itself holds no SQL string, `sqlx::query*` call, or connection (FR-F037-10, FR-F037-12).
...
- Rust service/API: `crates/domain/src/notifications/{preferences.rs, quiet_hours.rs, digest.rs}` (repository traits only, no SQLx); `crates/persistence/src/notifications/{mod.rs, notification_preference_repository.rs, digest_schedule_repository.rs}` holding the preference, channel-row, and schedule SQL; `services/api/src/notifications/{handlers_preferences.rs, dto.rs}`; `services/worker/src/notifications/{send_digest.rs, quiet_hours_release.rs}`
- Data/migration: `services/api/migrations/<ts>_notifications_preferences.sql` creating `notification_preferences` (typed `digest_cadence`, `digest_send_at_local`, `digest_timezone`, `quiet_hours_enabled`, `quiet_hours_start`, `quiet_hours_end`, `quiet_hours_timezone` columns with their check constraints), `notification_channel_preferences` (category and channel checks, `unique (preference_id, category, channel)`, index `(preference_id, enabled)`, `on delete cascade`), and `digest_schedules` with the cadence check, the `(tenant_id, coalesce(user_id, ...))` uniqueness, the `(tenant_id, user_id)` schedule uniqueness, and the `digest_schedules(next_run_at)` index
