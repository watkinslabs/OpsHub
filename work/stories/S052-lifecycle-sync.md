...
parent_feature: F026
depends_on: [S051]
owned_paths: [crates/domain/src/sso/**, services/api/src/sso/**, apps/web/src/features/sso/**, testing/features/F026/**]
...
- **SR-S052-03:** `group_mappings` map a SCIM group `external_id` or `display_name` to role IDs; group member add/remove/replace recomputes mapped role bindings in the same request and never removes manual bindings (FR-F026-14).
...
- Data/migration: none new; uses `group_mappings` and `scim_sync_log` from S051
