...
- **SR-S012-01:** `POST /api/v1/sheets/{id}/rows` creates a row with fractional `position`, default or given `group_id`, and cells keyed by column ID written through `RowRepository` and `CellRepository` in one transaction; returns `RowResponse` with version 1 (FR-F006-06).
- **SR-S012-02:** `GET /api/v1/sheets/{id}/rows` uses the named queries `list_rows_by_position` and `load_cells_for_rows` to return rows in `position` order with cursor paging and `limit` ≤ 500, including each cell's `raw` value, `display`, and typed `validation_state`, `validation_code`, and `validation_message` (FR-F006-07).
...
- Rust service/API: `crates/domain/src/sheets/{row.rs, position.rs, service_rows.rs}`; `services/api/src/sheets/{handlers_row.rs, handlers_group.rs}`; `rows` is written only by `RowRepository` and `cells` only by `CellRepository` (`crates/persistence/src/sheets/`), which own the named queries `list_rows_by_position` and `load_cells_for_rows`, so use cases, handlers, and tests hold no SQL (decision 2.1)
- Data/migration: none new; uses `sheets`, `sheet_settings`, `sheet_groups`, `rows`, and `cells` from S011
...
- First failing tests: `row_create_assigns_position`, `row_move_between_groups_emits_event`, `row_list_pages_by_position`, `row_cells_return_typed_validation_state`, `board_keyboard_move`, `row_list_100k_p95`
