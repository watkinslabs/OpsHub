...
parent_feature: F054
depends_on: [S107]
owned_paths: [crates/domain/src/bridge/**, services/api/src/bridge/**, services/worker/src/bridge/**, apps/web/src/features/bridge/**, testing/features/F054/**]
...
- Owner: platform
- Branch: `s108-run-operations`
- Decision references: `docs/architecture-decisions.md` sections 3, 4, 6, 7, 9; `docs/capability-contracts.md` row F054
...
- **SR-S108-01:** `GET /api/v1/bridge/runs` pages by cursor with `flow_id`, `status`, `from`, `to` filters and responds under 500 ms p95 with 100,000 runs; `GET /api/v1/bridge/runs/{id}` returns run, pinned `flow_version`, and ordered steps with redacted snapshots (covers FR-F054-11, NFR-F054-01).
- **SR-S108-02:** `POST /api/v1/bridge/runs/{id}/retry-step` re-executes a `failed` step from its input snapshot, resumes downstream, and emits `bridge-run.step-completed.v1`; a non-failed step → `409 conflict` (FR-F054-10).
- **SR-S108-03:** Cancel through the F019 route marks pending Bridge steps `cancelled` and the run `cancelled`; a `workflow-viewer` calling retry or cancel → `403 denied`; foreign tenant → `404 not_found` (FR-F054-14).
- **SR-S108-04:** `FlowBuilderPage` renders the step canvas with typed forms for the six step kinds, publish and run-now dialogs, and loading, empty, error, denied, stale, offline, and not-entitled states (FR-F054-15, FR-F054-13).
