...
owned_paths: [crates/domain/src/dashboards/**, services/api/src/dashboards/**, services/worker/src/dashboards/**, services/api/migrations/*_dashboards_*.sql, testing/features/F023/**]
...
- **SR-S045-01:** `POST /api/v1/dashboards` creates the dashboard with `layout`, `refresh_policy`, and empty widgets, returning `version` 1; duplicate names in a folder return `409 conflict` (FR-F023-01).
- **SR-S045-02:** `PUT /api/v1/dashboards/{id}/widgets` validates the twelve kinds and per-kind config through the registry, rejects overlaps, out-of-range positions, and more than 40 widgets with `400 invalid`, diffs against existing widgets keeping cache for retained IDs, and publishes `dashboard.updated.v1` (FR-F023-02, FR-F023-03).
...
- **SR-S045-04:** `GET /api/v1/widgets/{id}/data` serves `widget_cache` by `(widget_id, scope_key)`, enqueues a `dashboards.refresh-widget` job on a miss, computes `stale` from `source_versions`, and returns `denied` without payload when the viewer lacks access to the source (FR-F023-05).
- **SR-S045-05:** `POST /api/v1/dashboards/{id}/refresh` returns `202 { run_id, status, widget_count }` under 2 seconds, `409 conflict` while active, and the worker resolves widgets 8 at a time with a 20 s timeout, isolates failures, and publishes `dashboard.refreshed.v1` (FR-F023-06, NFR-F023-04).
- **SR-S045-06:** `GET /api/v1/dashboards`, `GET /api/v1/dashboards/{id}` with `cache_summary`, `PATCH`, and `DELETE` behave per FR-F023-08 and FR-F023-10, including cascade soft delete and foreign-tenant `404`.
...
- Data/migration: `services/api/migrations/<ts>_dashboards_create_tables.sql` creating `dashboards`, `dashboard_widgets`, `widget_cache` with the checks and indexes from ticket section 4
