...
parent_feature: F011
depends_on: [F007]
owned_paths: [crates/domain/src/schedules/**, services/api/src/schedules/**, services/api/migrations/*_schedules_*.sql, testing/features/F011/**]
...
- **SR-S021-02:** `POST /api/v1/working-calendars` creates a calendar with validated timezone, week intervals, and `hours_per_day`; the first `GET /api/v1/working-calendars` for a tenant materializes the `Standard` default (FR-F011-02).
- **SR-S021-03:** `PATCH /api/v1/working-calendars/{id}` requires `If-Match`, swaps the default atomically, and replaces the exceptions list with at most 400 unique dates (FR-F011-03, FR-F011-04).
- **SR-S021-04:** `add_working_days`, `working_days_between`, `snap_start`, and `snap_end` skip weekends and holiday exceptions and honour `working` exceptions; results are identical across DST transitions and leap days (FR-F011-07).
...
- Rust service/API: `crates/domain/src/schedules/{mod.rs, types.rs, calendar.rs, calendar_math.rs, errors.rs, service_calendar.rs}`; `services/api/src/schedules/{mod.rs, routes.rs, handlers_calendar.rs, dto.rs}`
- Data/migration: `services/api/migrations/<ts>_schedules_create_tables.sql` creating `working_calendars`, `calendar_exceptions`, `sheet_schedule_settings` with the indexes from ticket section 4
