...
owned_paths: [crates/domain/src/dependencies/**, services/api/src/dependencies/**, services/api/migrations/*_dependencies_*.sql, testing/features/F012/**]
...
- **SR-S023-01:** `POST /api/v1/dependencies` with `{ predecessor_row_id, successor_row_id, kind, lag?, lag_unit? }` inserts a `row_dependencies` row, returns `DependencyResponse` with version 1, and publishes `dependency.created.v1` (covers FR-F012-01).
...
- **SR-S023-03:** A candidate edge that closes a cycle returns `400 invalid` with `field_errors.successor_row_id = "cycle"` and `details.cycle_path`; the check runs under a per-sheet lock so concurrent inserts cannot both pass (FR-F012-03).
- **SR-S023-04:** A duplicate pair returns `409 conflict` with `details.existing_id`; the 20,001st dependency on a sheet returns `400 invalid` with `field_errors.sheet_id = "limit"` (FR-F012-04, FR-F012-05).
...
- **SR-S023-06:** `GET /api/v1/sheets/{sheet_id}/critical-path` runs the forward/backward pass for FS, SS, FF, SF with signed day and hour lag, rolls up parents, treats zero-duration rows as milestones, persists `schedule_results`, and returns `is_critical` for zero-float rows (FR-F012-08, FR-F012-09, FR-F012-10).
...
- Rust service/API: `crates/domain/src/dependencies/{mod.rs, dependency.rs, graph.rs, cycle.rs, critical_path.rs, errors.rs, service.rs}`; `services/api/src/dependencies/{mod.rs, routes.rs, handlers_dependency.rs, handlers_critical_path.rs, dto.rs}`
